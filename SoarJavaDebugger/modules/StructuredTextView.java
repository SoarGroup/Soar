...
import modules.FoldingTextView.RunWrapper;
...
/************************************************************************
 * 
 * Temp class -- included for speed tests against FoldingTextView.
 * This is just a simple text window dumping out the structured text data.
 * We should be able to go as fast at this window but probably not much
 * faster as it is paying the same cost to create the strings that the folding
 * view is, most of which appears to be below the debugger.
 * 
 ************************************************************************/
public class StructuredTextView extends AbstractComboView
{
	protected Text m_FoldingText ;
	...
	********************************************************************************************/
	...
	{
		m_FoldingText = new Text(parent, SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL | SWT.READ_ONLY) ;
		m_LastRoot = null ;
		
		createContextMenu(m_FoldingText) ;
...
//		menuItem.addSelectionListener(new SelectionAdapter() { public void widgetSelected(SelectionEvent e) { expandPage(false) ; } } ) ;
	...
	*************************************************************************/
	...
	{
		m_FoldingText.append(text + getLineSeparator()) ;
		/*
		String[] lines = text.split(getLineSeparator()) ;
		...
		{	
			...
			m_FoldingText.append(lines[i] + getLineSeparator()) ;
		}
		*/
	}
	...
	********************************************************************************************/
	...
	{
		StructuredTextView m_View ;
		Agent 		  	m_Agent ;
		ClientXML 	  	m_XML ;
		
		public RunWrapper(StructuredTextView view, Agent agent, ClientXML xml)
		{
			m_Agent = agent ;
			m_XML   = xml ;
			m_View  = view ;
