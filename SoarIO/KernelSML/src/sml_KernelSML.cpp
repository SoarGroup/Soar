...
void KernelSML::DeleteAllAgents(bool waitTillDeleted)
{
	...
	while (m_AgentMap.size() != 0)
	{
		...
		{
			soar_thread::Thread::SleepStatic(10) ;
			maxTries-- ;
		}
	}
}
...
EXPORT void sml_DirectRun(char const* pAgentName, int decisions, bool clearInterrupts)
{
	...
	// We usually need to clear interrupts to get rid of any previous interrupts (such as are generated by stop-on-output).
	if (clearInterrupts)
		pAgent->ClearInterrupts() ;
