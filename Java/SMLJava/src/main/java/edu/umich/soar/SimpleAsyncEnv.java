 ...
 * @author voigtjr
 * 
 *         A simple, asynchronous, interactive environment example. A loop is
 *         started that accepts input from standard in. The input is simply
 *         parsed and, unless it is a special command, it is asynchronously
 *         passed on to the Soar input link. If it is a special command, then
 *         the environment acts on the command.
 * 
 *         The special commands include start (or run) to start running Soar,
 *         stop to stop running Soar, and quit to exit the program.
 * 
 *         The Soar interface reports any non-special commands received on the
 *         command line to a messages WME on the input-link. Each message has a
 *         unique id. The agent is expected to issue commands on the
 *         output-link.
 * 
 *         One of these Soar commands is print, and it simply prints the
 *         contents of the "content" parameter. The other command is clear which
 *         tells the Soar interface to remove any existing messages from the
 *         input-link.
 * 
 *         The agent included with this registers a few canned responses with
 *         itself (responding to "hello" and "jump") and then uses these canned
 *         responses if their triggers come in. If it doesn't have a canned
 *         response then it simply echos the message as a command on the output
 *         link.
 * 
 *         Since the agent doesn't have a response for clear, issuing this means
 *         it gets passed as a command on the output link which then, in turn,
 *         causes the environment to clear the input link.
 *         
 *         Illustrative run:
 *         > start
 *         > 
 ...
 *         > quit
 *         
 *         Note that the Soar Java Debugger can connect to this environment as
 *         soon as the environment is started. Issue the -remote switch when
 *         launching the debugger. Avoid starting and stopping Soar from within
 *         the debugger when doing this, however, since this environment is not
 *         written to handle that correctly in all cases.
 *         
 *         This example would normally be implemented using multiple files
 *         rather than static inner classes but it is done like this in order to
 *         contain it in one file. These classes could be moved out and things
...
public class SimpleAsyncEnv {
	...
	public Soar() {
	     ...
	     */
	    try {
		String result = ProductionUtils.sourceAgentFromJar(agent, "/simpleasyncenv.soar");
	    ...
	    kernel.RegisterForUpdateEvent(smlUpdateEventId.smlEVENT_AFTER_ALL_OUTPUT_PHASES, new UpdateEventInterface() {
			...
			/*
			 * Pull each line out of the queue and post it on the input-link.
			 */
			for (String line = lines.poll(); line != null; line = lines.poll()) {
			    /*
			     * Each message has its own message WME off of the messages root.
			     */
			    Identifier message = messagesRoot.CreateIdWME("message");
			    /*
			     * On each message WME there are two attribute value pairs: one 
			     * is the integer id number of the message and the other is its 
			     * string content.
			     */
			...
			for (int index = 0; index < agent.GetNumberCommands(); ++index) {
			    /*
			     * Get the command by index. Note: avoid storing this identifier because 
			     * the agent created it and may delete it at any time. If you need to store
			     * it across decision cycles, you'll need to do a lot more work to make sure
			     * it is valid before attempting to use it in future updates.
			     */
			    Identifier command = agent.GetCommand(index);
			    
			    /*
			     * This is the attribute of the command identifier's WME.
			     */
			    String name = command.GetCommandName();
			    
			    if (name.equals(PRINT)) {
				/*
				 * Send print commands directly to the listener and add
				 * status complete.
				 */
				 ...
				 * Deleting a WME deletes all of its children (in this
				 * case the id and content WMEs).
				 */
				...
				/*
				 * Clear our cache now that it has been invalidated.
				 */
				messages.clear();
				...
				/*
				 * Issue error feedback message and mark status error..
				 */
			...
			/*
			 * This marks any commands on the output-link as seen so
			 * they will not be encountered via GetCommand on future
			 * updates if they are still on the output-link then.
			 */
			agent.ClearOutputLinkChanges();
			
			/*
			 * Finally, check to see if we have been asked to stop and issue a stop
			 * if so.
			 */
			if (stopSoar.get()) {
	...
	public SimpleConsole(Soar soar) {
		...
		public void printEvent(String message) {
		    /*
		     * Clearly distinguish output from the agent. In a GUI this would
		     * go to its own text box. Reprint the prompt since it is likely clobbered.
		     */
		    System.out.print(String.format("%nSoar interface: %s%n%s", message, PROMPT));
		}
	    });
	    
	    /*
	     * Start the input loop. Separated in to its own function call for clarity.
	     */
	    run();
	}
	...
	private void run() {
		...
		while (!Thread.interrupted()) {
			...
			/*
			 * Start Soar in its own thread. Soar implements Runnable
			 * which  means it will execute the run() method.
			 * 
			 * Note that multiple successive calls to this will cause
			 * the run calls to queue up in the executor service. Calling
			 * start twice then stop will cause soar to stop (when stop is
			 * issued) and then immediately start up again as the second
			 * start gets processed. All of this can be prevented by
			 * registering for system start and stop events with the kernel.
			 * See kernel.RegisterForSystemEvent()
			 */
			exec.execute(soar);
			
		    } else if (line.equalsIgnoreCase(STOP)) {
			/*
			 * Politely ask Soar to stop itself asynchronously.
			 * Does not block.
			 */
			soar.stop();
