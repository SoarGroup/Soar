...
public class TestJavaSML
{
	...
	public static class EventListener implements Agent.RunEventInterface, Agent.PrintEventInterface, Agent.ProductionEventInterface,
			Agent.xmlEventInterface, Agent.OutputEventInterface, Agent.OutputNotificationInterface, Kernel.AgentEventInterface,
			Kernel.SystemEventInterface, Kernel.RhsFunctionInterface
	{
		...
		// time this method is called. So instead we look up the Agent object from
		// the kernel with GetAgent().
		...
		public void xmlEventHandler(int eventID, Object data, Agent agent, ClientXML xml)
		{
			...
			// C++
			// where you need to copy the object to keep it, which means you
			// don't need to
	...
	private void Test()
	{
		...
		/*******************************************************
		 * // Register some event handlers // Each call takes an object from a class that implements a handler method // (E.g. listener has
		 * "runEventHandler"). // The type for that method is fixed by SML (there are examples above for each) // I'm using one listener
		 * class for all events but each could be in its own class or they // could be methods in "this" class. // The last parameter is an
		 * arbitrary data object which will be passed back to us in the callback. // I'm just passing in "this" but it could be anything. //
	...
	// results beforehand
	// The filename shows if things succeeded or not and the contents can
	// explain further.
