...
/************************************************************************
 * Simple test app for working out SML (Soar Markup Language) interface through
 * Java
 ************************************************************************/
public class TestJavaSML
{
    private Kernel m_Kernel;

    public static class EventListener implements Agent.RunEventInterface,
            ...
            Kernel.SystemEventInterface, Kernel.RhsFunctionInterface
    {
        ...
        public void runEventHandler(int eventID, Object data, Agent agent,
                int phase)
        {
            System.out.println("Received run event in Java");
            // TestJavaSML me = (TestJavaSML)data ;
        }

        // We pass back the agent's name because the Java Agent object may not
        // yet exist for this agent yet. The underlying C++ object *will* exist
        // by the time this method is called. So instead we look up the Agent
        // object from the kernel with GetAgent().
        public void agentEventHandler(int eventID, Object data, String agentName)
        {
            ...
            if (agent == null)
                throw new IllegalStateException(
                        "Error in agent event handler -- got event for agent that hasn't been created yet in ClientSML");
        }

        public void productionEventHandler(int eventID, Object data,
                Agent agent, String prodName, String instantiation)
        {
                ...
                ClientXML xml)
        {
            ...
            // No need to explicitly delete xml now. The Java version is like
            // C++ where you need to copy the object to keep it, which means you
            // don't need to explicitly delete it. xml.delete() ;
        }
        ...
        {
            System.out.println("Received output event in Java for wme: "
                    + pWmeAdded.GetIdentifierName() + " ^"
                    + pWmeAdded.GetAttribute() + " "
                    + pWmeAdded.GetValueAsString());

            // Retrieve the application class pointer (to test our user data was
            // passed successfully) and then print out the tree of wmes from the
            // wme added down.
            TestJavaSML app = (TestJavaSML) data;
    ...
    public void printWMEs(WMElement pRoot)
    {
        ...
        {
            System.out.println("(" + pRoot.GetParent().GetIdentifierSymbol()
                    + " ^" + pRoot.GetAttribute() + " "
                    + pRoot.GetValueAsString() + ")");
        }

        if (pRoot.IsIdentifier())
        {
            // NOTE: Can't just cast to (Identifier) in Java because the
            // underlying C++ object needs to be cast
    ...
    private void Test()
    {
        ...
        if (m_Kernel.HadError())
            throw new IllegalStateException("Error initializing kernel: "
        ...
        if (m_Kernel.HadError())
            throw new IllegalStateException("Error creating agent: "
                    + m_Kernel.GetLastErrorDescription());
        ...
        if (agent.HadError())
            throw new IllegalStateException(
                    "Error loading productions from testjavasml.soar: "
        ...
        StringElement pEmpty = pAgent.CreateStringWME(pSquare, "content",
                "RANDOM");
        // IntElement pRow =
        ...
        /*******************************************************
         * // Register some event handlers // Each call takes an object from a
         * class that implements a handler method // (E.g. listener has
         * "runEventHandler"). // The type for that method is fixed by SML
         * (there are examples above for each) // I'm using one listener class
         * for all events but each could be in its own class or they // could be
         * methods in "this" class. // The last parameter is an arbitrary data
         * object which will be passed back to us in the callback. // I'm just
         * passing in "this" but it could be anything. // The integer we get
         * back is only required when we unregister the handler.
         ********************************************************/
        EventListener listener = new EventListener();
        long jRunCallback = pAgent.RegisterForRunEvent(
                smlRunEventId.smlEVENT_AFTER_DECISION_CYCLE, listener, this);
        long jProdCallback = pAgent.RegisterForProductionEvent(
                smlProductionEventId.smlEVENT_AFTER_PRODUCTION_FIRED, listener,
                this);
        long jPrintCallback = pAgent.RegisterForPrintEvent(
                smlPrintEventId.smlEVENT_PRINT, listener, this);
        long jSystemCallback2 = pKernel.RegisterForSystemEvent(
                smlSystemEventId.smlEVENT_SYSTEM_START, listener, this);
        long jAgentCallback = pKernel.RegisterForAgentEvent(
        ...
        long jOutputNotification = pAgent.RegisterForOutputNotification(
                listener, this);
        ...
        // Now we should match (if we really loaded the tictactoe example rules)
        // and so generate some real output
        String trace = pAgent.RunSelfTilOutput(); // Should just cause Soar to
        // run a decision or two (this
        // is a test that run til
        // output works stops at
        // output)
        ...
        if (value == null)
            throw new IllegalStateException(
                    "RHS function failed to set a value for ^test");
        ...
        if (listener.m_Keep == null)
            throw new IllegalStateException(
                    "The xmlEventHandler wasn't called (so m_Keep is null)");

        // Try to access m_Keep to make sure the underlying XML object's not
        // been deleted
        ...
        if (!pAgent.UnregisterForOutputNotification(jOutputNotification))
            throw new IllegalStateException(
                    "Unregister failed: jOutputNotification");
        if (!pAgent.RemoveOutputHandler(jOutputCallback))
            throw new IllegalStateException(
                    "Unregister failed: jOutputCallback");
        if (!pKernel.UnregisterForSystemEvent(jSystemCallback2))
            throw new IllegalStateException(
                    "Unregister failed: jSystemCallback2");
        ...
        // bug 1028
        ElementXML element = ElementXML
                .ParseXMLFromString("<sml><result>=</result></sml>");
        if (element == null)
        {
            // failure
        ...
        // Delete the kernel, releasing all of the owned objects (hope this
        // works ok in Java...)
        m_Kernel.delete();
    }

    // We create a file to say we succeeded or not, deleting any existing
    // results beforehand. The filename shows if things succeeded or not and the
    // contents can explain further.
    public void reportResult(String testName, boolean success, String msg)
    {
        try
        {
            ...
            java.io.FileWriter outfile = new java.io.FileWriter(
                    success ? successFile : failFile);
        ...
        {
            System.out
                    .println("Exception occurred when writing results to file");
        }
    }

    private void TestListener() throws Exception
    {
        // Sleep for 20 seconds so others can connect to us if they wish to test
    ...
    {
        // Now repeat the tests but with a kernel that's running in a different
        // process (needs a listener to exist for this)
        System.out.println("############ Remote Test ############");

        // Initialize the remote kernel
        m_Kernel = Kernel.CreateRemoteConnection(true, null, Kernel
                .GetDefaultPort());

        if (m_Kernel.HadError())
        {
            System.out
                    .println("Failed to connect to the listener...was one running?");
            return;
        }
    ...
    public TestJavaSML(String[] args)
    {
        ...
        try
        {
            ...
            if (localTests)
            {
                // Now repeat the tests but with a kernel that's running in a
                // different thread
