...
public abstract class AbstractRHSFunView extends AbstractUpdateView implements
        Kernel.AgentEventInterface, Kernel.RhsFunctionInterface
{
        ...
        if (rhsFunName.length() > 0)
        {
            if (!this.m_Document.registerRHSFunction(rhsFunName, this, null))
            {
                ...
                MessageBox errorDialog = new MessageBox(
                        this.m_Frame.getShell(), SWT.ICON_ERROR | SWT.OK);
                errorDialog.setMessage("RHS function already registered: \""
                        + rhsFunName);
                errorDialog.open();
                rhsFunName = "";
            }
        ...
        if (rhsFunName.length() > 0)
        {
            ...
            if (!ok)
                throw new IllegalStateException(
                        "Problem unregistering for events");
        }
    }
    ...
    {
        rhsFunInitSoarHandler = agent.GetKernel().RegisterForAgentEvent(
                smlAgentEventId.smlEVENT_AFTER_AGENT_REINITIALIZED, this, this);
    }
    ...
    protected boolean unregisterForViewAgentEvents(Agent agent)
    {
        ...
        if (rhsFunInitSoarHandler != -1)
            ok = agent.GetKernel().UnregisterForAgentEvent(
                    rhsFunInitSoarHandler);
            ...
            ArrayList<PropertiesDialog.Property> properties)
    {
        ...
        properties
                .add(new PropertiesDialog.StringProperty(
                        "Name of RHS function to use to update this window",
                        rhsFunName));
        properties.add(new PropertiesDialog.BooleanProperty("Debug messages",
                debugMessages));
    }
            ...
            ArrayList<PropertiesDialog.Property> properties)
    {
        ...
        tempRHSFunName = ((PropertiesDialog.StringProperty) properties
                .get(propertiesStartingIndex)).getValue();
        tempRHSFunName = tempRHSFunName.trim();

        debugMessages = ((PropertiesDialog.BooleanProperty) properties
                .get(propertiesStartingIndex + 1)).getValue();
