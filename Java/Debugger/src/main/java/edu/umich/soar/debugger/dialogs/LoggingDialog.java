...
public class LoggingDialog extends BaseDialog
{
    ...
    public static LoggingDialog showDialog(MainFrame frame, String title,
            AbstractView view)
    {
    ...
    protected void createContents(final Composite window, Logger logger)
    {
        ...
        m_FlushAfterWrite
                .setText("&Flush after each write (bit slower but log always up to date)");
        ...
        m_Browse.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent event)
            {
                String filename = SaveLoad.SaveFileDialog(m_Frame.getWindow(),
                        new String[] { "*.txt" },
                        new String[] { "Log file (*.txt)" }, m_Frame
                                .getAppProperties(), "LogSave", "LogLoad");
