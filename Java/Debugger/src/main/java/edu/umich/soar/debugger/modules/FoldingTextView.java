...
import edu.umich.soar.debugger.helpers.FoldingText;
import edu.umich.soar.debugger.helpers.FormDataHelper;
import edu.umich.soar.debugger.helpers.TraceType;
        ...
        Agent.xmlEventInterface
{
    ...
    /**
     * When true, we expand the tree as it's created -- but this one is not
     * persistent between debugger sessions
     */
    protected boolean m_ExpandTrace = false;

    /**
     * When true, keep debugger and Soar locked together so the debugger's UI
     * doesn't get behind Soar. Tying the processes together usually makes Soar
     * run a lot slower, but sometimes that will be useful
     */
    protected boolean m_LockToSoar = false;

    /**
     * The last root (top level item) added to the tree. We add new sub items
     * under this
     */
    ...
    /**
     * Controls whether we cache strings that are due to be subtree nodes and
     * only add the nodes when the user clicks--or not
     */
    protected final static boolean kCacheSubText = true;

    /**
     * We use this structure if we're caching sub nodes in the tree for
     * expansion only when the user clicks
     */
    ...
    protected void updateButtonState()
    {
        ...
        m_ExpandButton.setData("expand", m_ExpandTrace ? Boolean.TRUE
                : Boolean.FALSE);
        ...
        // Change the color of the label if any filtering is enabled, so it's
        // clear that this is happening.
        // Don't want to be doing this by accident and not realize it.
        if (m_FoldingText.isFilteringEnabled()
                && m_FoldingText.getExclusionFilter() != 0)
        {
    ...
    protected void createDisplayControl(Composite parent)
    {
        ...
        m_FoldingText.getTextWindow().addKeyListener(new KeyAdapter()
        {
            ...
            {
                // Only for keys that generate characters (so arrows keys, shift
                // etc. can be safely pressed)
                        ...
                        || e.character == '!')
                {
                    ...
                    m_CommandCombo.setSelection(new Point(m_CommandCombo
                            .getText().length(), m_CommandCombo.getText()
                            .length()));
                }
            }
        });

        // Add a button that offers an expand/collapse option instantly (for
        // just one page)
        m_ExpandTrace = m_ExpandTracePersistent;
        m_ExpandButton = new Button(owner, SWT.PUSH);

        m_ExpandButton.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                ...
                // We expand the current page if the user asks for "expand" but
                // we're not making this symmetric
                // because "collapse" probably means
                // "I'm done with detailed debugging" but not necessarily
                // "I don't want to see what I was just working on".
        ...
        m_ExpandPageArrow.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent event)
            {
                Point pt = m_ExpandPageArrow.toDisplay(new Point(event.x,
                        event.y));
        ...
        // Note: Doing expand page then collapse page doesn't get you back to
        // where you started--the page will be showing far fewer
        // blocks when you hit "collapse page" so it does less work. Collapse
        // page may have little value.
        ...
        m_FilterArrow.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent event)
            {
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kPhase);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kPreference);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kWmeChange);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kFiring);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kRetraction);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kStack);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kLearning);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kFullLearning);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kVerbose);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                changeFilter(e.widget, TraceType.kWarning);
            }
        });
        ...
        menuItem.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                m_FoldingText.setExclusionFilter(0, true);
                updateButtonState();
            }
        });
        ...
        if (OSName.isNotWindowsAndNotMac())
        {
            m_FoldingText.getTextWindow().addPaintListener(new PaintListener()
            {
                ...
                {
                    // First time we are asked to paint, trigger a relaying out
                    // of the window.
                    // This is to fix a problem on Linux Motif version of SWT
                    // where the window fails to resize until manually adjusted.
                    // The reason to listen for paint is that we want to wait
                    // until the control is actually on screen before doing this
                    // resize.
                    // Then we submit a request to layout the window a little
                    // while later (maybe we could do it immediately, I'm not
                    // sure)
                    int delayMillis = 500;
            ...
            boolean wrap, boolean searchHiddenText)
    {
        ...
        // If we're searching the entire set of text need to switch to the
        // position within the entire set of text
        if (searchHiddenText)
            selectionStart = m_FoldingText
                    .convertVisibleToAllCharPos(selectionStart);
        ...
        do
        {
            ...
            if (start != -1)
            {
                ...
                if (wrapped
                        && ((searchDown && (start >= origStart)) || (!searchDown && (start <= origStart))))
                    found = false;
            }
            else
            {
        ...
        if (found)
        {
            ...
            // If we're searching in hidden text need to convert back to the
            // visible space and
            ...
            int mouseY)
    {
        ...
        // Sometimes we need to search back up the trace or down the trace to
        // determine the context
        // so we'll add these lines above and below the to current line,
        // adjusting the position as we do.
    ...
    public void setTextFont(Font f)
    {
        ...
        // Changing the font means we need to redraw the icon bar, just as if we
        // scrolled
    ...
    protected void appendSubText(String text, long type)
    {
        ...
        for (int i = 0; i < lines.length; i++)
        {
            ...
            m_FoldingText.appendSubText(lines[i] + kLineSeparator,
                    m_ExpandTracePersistent || m_ExpandTrace, type);
     ...
     * |Hello|) but switch it over to end terminated here. This has the effect
     * that a single (write | Hello |) will now not appear on the end of the
     * previous line of trace output (as it would have in earlier Soars). (This
     * was generally just a slip anyway but now it won't be possible because the
     * previous line of output in the trace has already added a trailing line
     * separator). But we do still want to allow a series of writes to build up
     * some text (e.g. writing out a board position). To make that work we
     * collect up all RHS writes that occur sequentially and only then process
     * them. This avoids us inserting unwanted line separators between writes
    ...
    protected boolean isNextChildRhsWrite(ClientXML xmlParent, int childIndex,
            int nChildren)
    {
    ...
    protected void displayXmlTraceEvent(Agent agent, ClientXML xmlParent)
    {
        ...
        for (int childIndex = 0; childIndex < nChildren; childIndex++)
        {
            ...
            {
                String text = XmlOutput.getStateText(agent, xmlTrace,
                        m_IndentSize);
            ...
            {
                String text = XmlOutput.getOperatorText(agent, xmlTrace,
                        m_IndentSize);
            ...
            else if (xmlTrace.IsTagRhsWrite())
            {
                ...
                // (This lets us support writes that don't start with a leading
                // (crlf) correctly)
                if (output.length() != 0)
                {
                    rhsWrites.append(output);

                    // When we reach the end of a sequence of RHS writes process
                    // them all
            ...
            else if (xmlTrace.IsTagPhase())
            {
                ...
                // BUGBUG? Perhaps we should include them now and just use the
                // filtering to remove them
                boolean endOfPhase = (status != null && status
            ...
            else if (xmlTrace.IsTagAddWme() || xmlTrace.IsTagRemoveWme())
            {
                ...
                String output = XmlOutput
                        .getWmeChanges(agent, xmlTrace, adding);
            ...
            {
                String output = XmlOutput.getPreferenceProductionText(agent,
                        xmlTrace);
            ...
            else if (xmlTrace.IsTagFiringProduction()
                    || xmlTrace.IsTagRetractingProduction())
            {
                boolean firing = xmlTrace.IsTagFiringProduction();

                String output = XmlOutput.getProductionFiring(agent, xmlTrace,
                        firing);
            ...
            else if (xmlTrace.IsTagLearning())
            {
                ...
                for (int i = 0; i < xmlTrace.GetNumberChildren(); i++)
                {
                    ...
                    if (prod.IsTagProduction())
                    {
                        ...
                        {
                            // If has children we're looking at a full
                            // production print
                            // (Note -- we get the "we're building" from above
                            // as well as this so we don't
                            // add the "build x" message when this comes in.
                            String prodText = XmlOutput.getProductionText(
                                    agent, prod);

                            if (prodText != null && prodText.length() != 0)
                                this.appendSubText(prodText,
                                        TraceType.kFullLearning);
                        }
                    }
                    prod.delete();
                }
            ...
            else if (xmlTrace.IsTagCandidate())
            {
                ...
                String output = XmlOutput.getNumericIndiffernceText(agent,
                        xmlTrace);
                ...
                if (output.length() != 0)
                    ...
                    || xmlTrace.IsTagGroundedPotentials()
                    || xmlTrace.IsTagUngroundedPotentials())
            {
                String output = XmlOutput.getBacktraceText(agent, xmlTrace,
                        xmlTrace.GetTagName());
            ...
            {
                String output = XmlOutput.getLocalNegationText(agent, xmlTrace,
                        xmlTrace.GetTagName());
        ...
        // Technically this will happen when the object is garbage collected
        // (and finalized)
        // but without it we'll get a million memory leak messages on shutdown
        // because (a) gc may not have been run for a while
        // (b) even if it runs not all objects will be reclaimed and (c)
        // finalize isn't guaranteed before we exit
    ...
    public void xmlEventHandler(int eventID, Object data, Agent agent,
            ClientXML xml)
    {
        if (eventID != smlXMLEventId.smlEVENT_XML_TRACE_OUTPUT.swigValue())
            return;

        // The messages come collected into a parent <trace> tag so that one
        // event
        ...
        // Have to make update in the UI thread.
        // Callback comes in the document thread.
        // NOTE: I had to write a real class here to do this rather than just
        // using my
        // "normal" trick of making agent and xml "final" and creating a class
        // on the fly.
        // Doing that lead to an intermittement memory leak from the xml
        // object--really hard to track down
        // and I'm still not fully clear on why that happened, but I suspect if
        // this event was called again
        // before the wrapper had run, we had a problem.

        // We need to create a new copy of the XML we were passed because we're
        // going to use an asynch call, which won't execute until after this
        // function has
        // completed and xml goes out of scope. Why use the asynch method rather
        // than syncExec()?
        // Using asynch here lets things run up to 20 times faster(!)
        // March 2006 update: We've now allowed this to become a synch call,
    ...
    public void showProperties()
    {
        ...
        // negative value
        properties[0] = new PropertiesDialog.IntProperty("Indent per subgoal",
                m_IndentSize, 0, 10);
        properties[1] = new PropertiesDialog.BooleanProperty(
                "Expand trace as it is created", m_ExpandTracePersistent);
        properties[2] = new PropertiesDialog.BooleanProperty(
                "Capture data to support filtering", m_FoldingText
                        .isFilteringEnabled());
        properties[3] = new PropertiesDialog.BooleanProperty(
                "Keep Soar locked to the speed of the debugger (makes Soar a lot slower)",
                m_LockToSoar);
        ...
        if (ok)
        {
            m_IndentSize = ((PropertiesDialog.IntProperty) properties[0])
                    .getValue();
            m_ExpandTracePersistent = ((PropertiesDialog.BooleanProperty) properties[1])
                    .getValue();
            m_FoldingText
                    .setFilteringEnabled(((PropertiesDialog.BooleanProperty) properties[2])
                            .getValue());
            m_LockToSoar = ((PropertiesDialog.BooleanProperty) properties[3])
                    .getValue();
            ...
            String tagName, boolean storeContent)
    {
        ...
        element.addAttribute("auto-expand", Boolean
                .toString(m_ExpandTracePersistent));
        element.addAttribute("lock-to-soar", Boolean.toString(m_LockToSoar));

        if (m_FoldingText != null)
        {
            element.addAttribute("filtering", Boolean.toString(m_FoldingText
                    .isFilteringEnabled()));
            element.addAttribute("filter", Long.toString(m_FoldingText
                    .getExclusionFilter()));
        }
    ...
    public void loadFromXML(MainFrame frame,
            ...
            throws Exception
    {
        // On Linux the SWT implementations are so slow Soar can get far to far
        // ahead
        // of the debugger to the point where it's essentially impossible to
        // stop soar
        ...
        m_ExpandTracePersistent = element.getAttributeBooleanDefault(
                "auto-expand", false);
        m_LockToSoar = element.getAttributeBooleanDefault("lock-to-soar",
                lockDefault);

        boolean filtering = element.getAttributeBooleanDefault("filtering",
                true);
        long filter = element.getAttributeLongDefault("filter", 0);

        super.loadFromXML(frame, doc, parent, element);

        // Have to wait until base class has been called and window has been
        // created before setting these values
        if (m_FoldingText != null)
        {
