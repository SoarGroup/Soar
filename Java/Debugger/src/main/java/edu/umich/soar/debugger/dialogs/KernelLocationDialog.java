...
public class KernelLocationDialog extends BaseDialog
{
    ...
    public static String showDialog(MainFrame frame, String title)
    {
        ...
        dialog.getDialog().setSize(dialog.getDialog().getSize().x * 85 / 100,
                dialog.getDialog().getSize().y * 120 / 100);
    ...
    private KernelLocationDialog(MainFrame frame, String title)
    {
        ...
        m_KernelFilename.addModifyListener(new ModifyListener()
        {
        ...
        // Set the initial default value, but allow this to be overridden from
        // an earlier saved setting
        kernelPathModified();
        ...
        m_Browse2.addSelectionListener(new SelectionAdapter()
        {
            public void widgetSelected(SelectionEvent e)
            {
                browse2Pressed();
    ...
    {
        String filename = SaveLoad.LoadFileDialog(getDialog(),
                new String[] { "*.dll" },
                new String[] { "Soar Kernel SML (*.dll)" }, m_Frame
                        .getAppProperties(), "KernelSave", "KernelLoad");
    ...
    {
        String folder = SaveLoad.SelectFolderDialog(getDialog(),
                m_LibraryFilename.getText(), "Select Soar library location",
                "Select the folder that contains the Soar library");
    ...
    protected void endDialog(boolean ok)
    {
        ...
        else
        {
            ...
            if (libraryLocation != null && libraryLocation.length() > 0)
                m_Frame.executeCommandPrimeView(m_Frame.getDocument()
                        .getSoarCommands().setLibraryLocationCommand(
