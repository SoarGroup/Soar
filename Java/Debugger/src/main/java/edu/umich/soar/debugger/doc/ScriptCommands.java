...
public class ScriptCommands
{
    ...
    // Return value is currently null, but we'll allow for it to return values
    // in the future.
    ...
    protected Object executeRemoteConnect(String[] tokens)
    {
        ...
        String portStr = tokens.length >= 3 ? tokens[2] : Integer
                .toString(Kernel.GetDefaultPort());
        ...
        {
            m_Frame.ShowMessageBox("Error making remote connection: "
                    + ex.getMessage());
            return Boolean.FALSE;
        }
    }
    ...
    protected Object executeAddView(String[] tokens)
    {
        ...
        Module module = NewWindowDialog.showDialog(frame, "Select new window",
                m_Document.getModuleList());
        if (module == null)
            return null;

        // Create an instance of the class. This view won't be fully
        // instantiated
        ...
        // (We do this because SWT requires us to rebuild all of the windows
        // from
        // the point of the change down, so we may as well rebuild everything
        // since we
        // already have all of that code and it's both fairly complex and
        // heavily used).
        ...
        // another level.
        // Later adjustment is to allow two fixed windows to be added next to
        // each other
        ...
        // If we're adding to the left side we assume the orientation is
        // vertical
        // If we're adding to the top we assume the orientation is horizontal
        boolean horiz = ((direction.equals(MainWindow.kAttachTopValue) || direction
                .equals(MainWindow.kAttachBottomValue)));

        // We create a new pane but with no parent window (so the pane object is
        // not instantiated
        ...
        if (addSash)
        {
            ...
            int orientation = (direction.equals(MainWindow.kAttachLeftValue) || direction
                    .equals(MainWindow.kAttachRightValue)) ? SWT.HORIZONTAL
                    : SWT.VERTICAL;
            int[] weights = new int[] { 50, 50 };
            newParent = frame.getMainWindow().buildXMLForSashForm(orientation,
                    weights);

            // The order to add the children depends on which side
            // we're adding to.
            if (direction.equals(MainWindow.kAttachRightValue)
                    || direction.equals(MainWindow.kAttachBottomValue))
            {
    ...
    protected Object executeAddTab(String[] tokens)
    {
        ...
        {
            frame
                    .ShowMessageBox("I'm sorry.  You can't add a fixed size window (like a button bar) to a window containing a variable sized one (like a text window)");
            return null;
        }
        ...
        {
            frame
                    .ShowMessageBox("I'm sorry.  You can't add a variable sized window (like a text window) to a window containing a fixed sized one (like a button bar)");
            return null;
        }
        ...
        // We do this dance because SWT doesn't generally allow the parent of a
        // window
        // to be changed after it was created.

        boolean storeContent = true; // Capture existing output so we can
                                     // redisplay it
        JavaElementXML xml = frame.getMainWindow().convertToXML(storeContent);
        ...
        JavaElementXML newViewXML = newView.convertToXML(AbstractView.kTagView,
                false);
    ...
    protected Object executeReplaceView(String[] tokens)
    {
        ...
        {
            frame
                    .ShowMessageBox("I'm sorry.  You can't replace a fixed size window (like a button bar) with a variable sized one (like a text window)");
            return null;
        }
        ...
        {
            frame
                    .ShowMessageBox("I'm sorry.  You can't replace a variable sized window (like a text window) with a fixed sized one (like a button bar)");
            return null;
        }

        // The plan is to build the XML for the current set of windows.
        // Then swap the part of the XML for the current pane with a new pane
        // based on the new window and then rebuild everything from the modified
        // XML.
        // We do this dance because SWT doesn't generally allow the parent of a
        // window
        // to be changed after it was created.
    ...
    // Move tabs in a tabbed view: movetabs <framename> <viewname> <top |
    // bottom>
    protected Object executeMoveTabs(String[] tokens)
    {
        ...
        existingPane.addAttribute(Pane.kAttributeTabAtTop, Boolean
                .toString(top));
    ...
    protected Object executeRemoveView(String[] tokens)
    {
        ...
        else
        {
            ...
            // If we're removing a fixed size view we want to just remove the
            // parent
            // composite to leave just the other pane in the pair.
            // If we're removing a variable size view and its part of a
            // composite
            // we delete the other (fixed size) pane as well which can lead to a
            // chain.
            boolean removingFixedSize = view.isFixedSizeView();

            if (removingFixedSize)
            {
                // With a fixed size window we just promote the other side of
                // the pair
                ...
                if (parent.getNumberChildren() != 2)
                    throw new IllegalStateException(
                            "The parent of a fixed size window should be a composite pair");
            ...
            {
                // With a variable sized window, if we're removing it from a
                // pair
                ...
                while (!done)
                {
                    ...
                    // If the parent is a composite, remove it and its twin for
                    // now
                    ...
                    {
                        // Note: For sashform's this actually leaves the XML in
                        // an invalid state with
                        // incorrect weight information (to match the wrong
                        // number of children).
                        // Fixed that by allowing the load logic to recover from
                        // that state and just
    ...
    protected Object executeRenameView(String[] tokens)
    {
        ...
        while (!done)
        {
            String name = frame.ShowInputDialog("Rename window",
                    "Enter the new name:", view.getName());

            // Check if user cancelled
            if (name == null || name.length() == 0
                    || name.equals(view.getName()))
                return null;

            // We can't allow spaces (throws off our script passing) or special
            // characters (might throw off XML load/save)
            // If we really cared about these we could relax all of these
            // limits, but it doesn't seem that critical.
            boolean valid = true;
            ...
            {
                frame
                        .ShowMessageBox("Invalid characters in name",
                                "Window names are limited to letters, numbers and '_', '-', '<', '>'");
                continue;
            }

            if (frame.isViewNameInUse(name))
            {
                frame.ShowMessageBox("Name in use",
                        "The window name has to be unique and " + name
                                + " is already in use.");
                continue;
            }
