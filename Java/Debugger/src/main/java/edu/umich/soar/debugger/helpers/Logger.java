...
public class Logger
{
    ...
    // Path to folder to use for log file. Store this separately because we can
    // cache this between runs but the filenames
    // we want to generate to ensure they remain unique across multiple debugger
    // frames and windows.
    ...
    // When true, flush the file after each write so we can examine it during a
    // run (otherwise it'll only update when the file closes)
    protected boolean m_FlushEachWrite = true;

    // When true, we should append to the log file. This can be overridden by a
    // direct call to startLogging().
    ...
    public void setView(AbstractView view)
    {
        ...
        // Create a filename that is unique to this window (view's name is only
        // unique within this main frame so we need both parts).
        m_LogFilename = "log_" + view.getName() + "_"
                + view.getMainFrame().getName() + ".txt";
    }
    ...
    {
        return m_FlushEachWrite;
    }

    public void setFlush(boolean state)
    {
        m_FlushEachWrite = state;
    }
    ...
    public String startLogging(boolean append)
    {
        if (m_View == null)
            throw new IllegalStateException(
                    "Need to initialize view before starting to log");
     ...
     *            Some strings use Soar (\n) newlines. If true we'll replace
     *            those with system newlines (e.g. \n\r on Windows)
     * @param appendNewline
     *            If true, append a system newline to the end of the text.
     ********************************************************************************************/
    public void log(String text, boolean replaceSoarNewLines,
    ...
    {
        LoggingDialog.showDialog(m_View.getMainFrame(), "Log this window",
                m_View);
    }
    ...
    public void loadFromXML(edu.umich.soar.debugger.doc.Document doc,
            ...
            throws Exception
    {
        // Check that we have this data -- this is just helpful when adding this
        // feature as existing layout files won't have these fields.
