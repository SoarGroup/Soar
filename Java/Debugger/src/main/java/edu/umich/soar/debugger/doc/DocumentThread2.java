...
public class DocumentThread2 extends Thread
{
    ...
    public abstract static class Command
    {
        ...
        /**
         * Execute method is called from the DocumentThread when this command
         * reaches the top of the queue. It MUST call recordResult() to report
         * the result of the command and to signal that the command has
         * completed.
         */
    ...
    public static class CommandExecCommandLineXML extends Command
    {
        ...
        protected void execute()
        {
            ...
            boolean success = m_Agent.ExecuteCommandLineXML(m_Command,
                    m_Response);
    ...
    public void executePending(String source)
    {
        ...
        while ((command = popNextCommand()) != null)
        {
            ...
            if (kTraceCommands)
                System.out.println("About to execute " + command + " from "
                        + source);

            // Execute the command. Note -- this call is not synchronized as it
            // may be a call to "run" and we need to be able
