...
import edu.umich.soar.debugger.dialogs.SelectAgentDialog;
...
public class AgentMenu
{
    ...
    /** Create this menu */
    public static AgentMenu createMenu(MainFrame frame, Document doc,
            String title)
    {
    ...
    public void updateMenu()
    {
        ...
        m_SelectAgent.setEnabled(m_Document.isConnected()
                && m_Document.getNumberAgents() > 0);
        m_CreateAgentSame.setEnabled(m_Document.isConnected());
        m_CreateAgentNew.setEnabled(m_Document.isConnected());
        m_DestroyAgent.setEnabled(m_Document.isConnected()
                && m_Frame.getAgentFocus() != null);
    ...
    {
        return m_Document.getAppProperties().getAppBooleanProperty(
                Document.kCreateNewWindowProperty, true);
    }
    ...
    // BUGBUG: We should change the menu items on all of the menus to match this
    // or
    // better look up the state when the menu is popped down. Right now turning
    // this off on
    // one menu won't change the menus on other frames.
    private void createNewOnAgent(ActionEvent e)
    {
        m_Document.getAppProperties().setAppProperty(
                Document.kCreateNewWindowProperty,
                m_CreateNewOnAgent.isChecked());
    }

    private void closeOnDestroy(ActionEvent e)
    {
        m_Document.getAppProperties().setAppProperty(
                Document.kCloseOnDestroyProperty, m_CloseOnDestroy.isChecked());
    }
    ...
    private void selectAgent(ActionEvent e)
    {
        if (m_Document.getNumberAgents() == 0)
        {
            throw new IllegalStateException(
                    "Menu item selectAgent should have been disabled");
        }

        // Ask the user to pick an agent
        String agentName = SelectAgentDialog.showDialog(m_Frame.getWindow(),
                "Select the current agent", m_Document.getAgentNameArray());
    ...
    private void createAgent(ActionEvent e, boolean newWindow)
    {
        ...
        String name = m_Frame.ShowInputDialog("Create Agent",
        ...
        if (newWindow)
        {
            MainFrame frame = MainFrame.createNewFrame(m_Frame.getDisplay(),
                    m_Document);
    ...
    private void destroyAgent(ActionEvent e)
    {
        ...
        {
            throw new IllegalStateException(
                    "Menu item destroyAgent should have been disabled");
        }
