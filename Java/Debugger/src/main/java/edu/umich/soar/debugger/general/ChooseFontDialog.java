...
public class ChooseFontDialog extends JPanel
{
    ...
    private FontDisplay[] m_FixedFontDisplays; // The list of just those that
                                               // are fixed width
    ...
    {
        // The default fonts from "getAllFonts()" are size 1 so I'm not
        // confident
        // about the char width info we'll get from them, so let's derive a
        // decent
    ...
    private void Init()
    {
        ...
        // the list of fixed list fonts if the user selects the check box as
        // this
        // process is slow.
        ...
        // We have to make the panel visible before we select the font name in
        // the list
    ...
    private void setSelectionFromFont(Font font)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            FontStyleDisplay style = (FontStyleDisplay) m_StyleList.getModel()
                    .getElementAt(i);
        ...
        for (int i = 0; i < n; i++)
        {
            FontDisplay fd = (FontDisplay) m_FontList.getModel()
                    .getElementAt(i);
    ...
    private void selectionChanged()
    {
        ...
        FontStyleDisplay styleDisplay = (FontStyleDisplay) m_StyleList
                .getSelectedValue();
        ...
        {
            // The current entry in the size field is not an integer, so ignore
            // it or
            // for some reason we couldn't create the font, so again ignore the
            // inputs.
            return;
        }
    ...
    public static Font ShowDialog(Frame frame, String title, Font initialFont)
    {
        ...
        dialog.setLocation(frame.getWidth() / 2 - width / 2, frame.getHeight()
                / 2 - height / 2);
