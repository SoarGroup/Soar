...
public class NewWindowDialog extends BaseDialog
{
     ...
     * @param modules
     *            The list of modules the user should select from
     * @return The value the user entered or null if they cancelled the dialog
     ********************************************************************************************/
    public static Module showDialog(MainFrame frame, String title,
            ModuleList modules)
    {
    ...
    private NewWindowDialog(MainFrame frame, String title, ModuleList modules)
    {
        ...
        m_Description = new Text(getOpenArea(), SWT.MULTI | SWT.WRAP
                | SWT.V_SCROLL | SWT.BORDER);

        // Create the list of agents and anchor it to the top
        m_Modules = new List(getOpenArea(), SWT.SINGLE | SWT.V_SCROLL
                | SWT.BORDER);
