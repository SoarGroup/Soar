        ...
        Kernel.RhsFunctionInterface
{
    ...
    protected void updateNow()
    {
        ...
        while (iter.hasNext())
        {
            ...
            // verify it exists
            String result = agent.ExecuteCommandLine("print "
                    + orderedIdentifier.getIdentifier());
            ...
            else
            {
                                        ...
                                        .toUpperCase() + ")"))
                {
                    ...
                    // -----
                    // As you'll recall, we remove old objects from the object
                    // window when
                    // their ids can no longer be printed. Unfortunately, there
                    // appears to be
                    // a "bug" in Soar (I use that term loosely because I don't
                    // really
                    // understand what's going on). Sometimes when the id is no
                    // longer
                    // connected to the state, they can still be printed (but
                    // show up as
                    ...
                    // (E27)
                    //
                    // We should detect this case and treat it the same as if it
                    // couldn't be
                    // printed. So if, when we do the print check, we just get
                    // the id in
                    // parentheses, then it should be removed. I can imagine
                    // someone actually
                    // having legitimate ids like this that they want to show in
                    // the window,
                    // but I think we should design this for the other 99.999%
                    // of cases :)
                    // -----
    ...
    public class OrderedIdentifier implements Comparable<OrderedIdentifier>
    {
        ...
        public int compareTo(OrderedIdentifier other)
        {
            ...
            // System.out.println("*** comparing " + this.id + " to " +
            // other.id);
            ...
            String agentName, String functionName, String argument)
    {
        ...
        if (commandLine[1].equals("--order"))
        {
            ...
            {
                return m_Name + ":" + functionName
                        + ": --order requires an integer argument";
            }

            OrderedIdentifier orderedIdentifier = idToOrdered
                    .get(commandLine[0]);
            ...
            return debugMessages ? m_Name + ":" + functionName
                    + ": Updated order for " + commandLine[0] : "";
        }
