...
{
    /**
     * The document is used to represent the Soar process. There is one shared
     * document for the entire debugger.
     **/
    ...
    /**
     * The name of this view -- this name is unique within the frame (not the
     * entire debugger) so we can cross-reference based on this name
     */
     ...
     * Unless you're sure, you usually want the other one (kLineSeparator) when
     * working with Soar commands/output.
     **/
    ...
    /********************************************************
     * All AbstractView's need to have a default constructor as that's how we
     * build them (using reflection) when loading/saving or picking up module
     * plug-ins.
     *********************************************************/
    public AbstractView()
    {
    ...
    public void generateName(MainFrame frame)
    {
        if (m_Name != null)
            throw new IllegalStateException(
                    "Should only call this once.  If we really want to allow this then if m_Name != null unregister this name before generating a new name.");

        // Use the frame that's passed in, in case our frame pointer is yet to
        // be initialized
            ...
            boolean includeCopyPaste)
    {
        ...
        // Copy and paste where paste really means to paste into the command
        // stream, not into the window.
        if (includeCopyPaste)
        {
            addItem(menu, "Copy", "copy " + m_Frame.getName() + " "
                    + this.getName());
            addItem(menu, "Paste", "paste " + m_Frame.getName() + " "
                    + this.getName());
            new MenuItem(menu, SWT.SEPARATOR);
        }

        addItem(menu, "Properties ...", "properties " + m_Frame.getName() + " "
                + this.getName());

        if (offerLogging())
        {
            if (!m_Logger.isLogging())
                addItem(menu, "Log this window...", "log dialog "
                        + m_Frame.getName() + " " + this.getName());
            else
                addItem(menu, "Stop logging to " + m_Logger.getFilePath(),
                        "log stop " + m_Frame.getName() + " " + this.getName());
        }

        if (offerClearDisplay())
            addItem(menu, "Clear window", "clear " + m_Frame.getName() + " "
                    + this.getName());

        new MenuItem(menu, SWT.SEPARATOR);
        addItem(menu, "Add tab ...", "addtab " + m_Frame.getName() + " "
                + this.getName());
        addItem(menu, "Add window to right ...", "addview " + m_Frame.getName()
                + " " + this.getName() + " " + MainWindow.kAttachRightValue);
        addItem(menu, "Add window to left ...", "addview " + m_Frame.getName()
                + " " + this.getName() + " " + MainWindow.kAttachLeftValue);
        addItem(menu, "Add window to top ...", "addview " + m_Frame.getName()
                + " " + this.getName() + " " + MainWindow.kAttachTopValue);
        addItem(menu, "Add window to bottom ...", "addview "
                + m_Frame.getName() + " " + this.getName() + " "
                + MainWindow.kAttachBottomValue);

        // Some commands are only if we're showing tabs (so names are visible)
        if (!getPane().isSingleView())
        {
            if (getPane().isTabAtTop())
                addItem(menu, "Move tabs to bottom", "movetabs "
                        + m_Frame.getName() + " " + this.getName() + " bottom");
            else
                addItem(menu, "Move tabs to top", "movetabs "
                        + m_Frame.getName() + " " + this.getName() + " top");
        }
        new MenuItem(menu, SWT.SEPARATOR);
        addItem(menu, "Replace window ...", "replaceview " + m_Frame.getName()
                + " " + this.getName());
        addItem(menu, "Rename window ...", "renameview " + m_Frame.getName()
                + " " + this.getName());
        new MenuItem(menu, SWT.SEPARATOR);
        addItem(menu, "Remove window", "removeview " + m_Frame.getName() + " "
                + this.getName());
    }
    ...
    protected Menu createContextMenu(final Control control)
    {
        ...
        menu.addMenuListener(new MenuListener()
        {
            public void menuShown(MenuEvent e)
            {
                ...
                // We'll build the menu dynamically based on the text the user
                // selects etc.
