        ...
        Kernel.AgentEventInterface, Kernel.SystemEventInterface
{
    ...
    protected String[] m_ImageNames = { "phase-none.png", "phase-input.png",
            ...
            "phase-stop-cursor-shadow2.png", "phase-cursor.png" };
    ...
    public void init(MainFrame frame, Document doc, Pane parentPane)
    {
        ...
        for (int i = 0; i < m_ImageNames.length; i++)
        {
            try
            {
                // Load the images from the input subfolder (either from disk
                // relative to the app or from the JAR)
    ...
    protected void createPanel(final Composite parent)
    {
        ...
        canvas.setSize(w, h + 8); // BADBAD: Presumably this +4 is a margin from
                                  // somewhere
    ...
    protected void mouseClicked(MouseEvent e)
    {
        ...
        String command = m_Document.getSoarCommands().getStopBeforeCommand(
                m_StopBeforePhase);
    ...
    protected void paintPhaseDiagram(GC gc, int imageIndex)
    {
        ...
        // The cursors extend a touch below the images so we need to clean up
        // the overlapping area
        gc.setBackground(canvas.getDisplay().getSystemColor(SWT.COLOR_WHITE));
        gc.fillRectangle(0, m_PhaseImages[imageIndex].getImageData().height,
                canvas.getClientArea().width, 5);
    ...
    public void systemEventHandler(int eventID, Object data, Kernel kernel)
    {
        ...
        if (eventID == smlSystemEventId.smlEVENT_SYSTEM_PROPERTY_CHANGED
                .swigValue())
        {
            updateNow(true);
        }
            ...
            edu.umich.soar.debugger.doc.events.AgentFocusEvent ev)
    {
        ...
        // When an agent is assigned to this window, update the control to show
        // the
    ...
    protected void registerForAgentEvents(Agent agent)
    {
        if (m_StopCallback == -1)
        {
            // Update on start, stop and on init-soar. Also listen for any time
            // the user types "set-stop" somewhere else.
            m_StartCallback = agent.GetKernel().RegisterForSystemEvent(
                    smlSystemEventId.smlEVENT_SYSTEM_START, this, this);
            m_StopCallback = agent.GetKernel().RegisterForSystemEvent(
                    smlSystemEventId.smlEVENT_SYSTEM_STOP, this, this);
            m_PropertyCallback = agent.GetKernel().RegisterForSystemEvent(
                    smlSystemEventId.smlEVENT_SYSTEM_PROPERTY_CHANGED, this,
                    this);
    ...
    protected void unregisterForAgentEvents(Agent agent)
    {
        ...
        if (m_PropertyCallback != -1)
            ok = agent.GetKernel().UnregisterForSystemEvent(m_PropertyCallback)
                    && ok;

        if (!ok)
            throw new IllegalStateException(
                    "Error unregistering callbacks in phase view");
    ...
    public void showProperties()
    {
        ...
        properties[0] = new PropertiesDialog.BooleanProperty(
                "Show phase marker between phases (not within a phase)",
                m_ShowMarkerBetweenPhases);
        properties[1] = new PropertiesDialog.BooleanProperty(
                "Use hourglass shaped cursor set", m_HourglassCursors);
        ...
        if (ok)
        {
            m_ShowMarkerBetweenPhases = ((PropertiesDialog.BooleanProperty) properties[0])
                    .getValue();
            m_HourglassCursors = ((PropertiesDialog.BooleanProperty) properties[1])
                    .getValue();

            // Update the cursor set and redraw the display to match the users
            // choices.
            selectCursorImages();
    ...
    public JavaElementXML convertToXML(String tagName, boolean storeContent)
    {
        ...
        element.addAttribute("BetweenPhases", Boolean
                .toString(m_ShowMarkerBetweenPhases));
        element.addAttribute("HourglassCursors", Boolean
                .toString(m_HourglassCursors));
            ...
            throws Exception
    {
        ...
        m_ShowMarkerBetweenPhases = element.getAttributeBooleanDefault(
                "BetweenPhases", true);
        m_HourglassCursors = element.getAttributeBooleanDefault(
                "HourglassCursors", true);
