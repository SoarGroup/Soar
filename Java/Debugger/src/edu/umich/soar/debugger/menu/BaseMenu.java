 ...
 * more easily without re-writing the existing code. Abstracting this logic is
...
public class BaseMenu
{
    ...
    public BaseMenu(Menu bar, String title, final MenuUpdater updater)
    {
        ...
        m_Header.addArmListener(e -> {

            // Give the menu a chance to change the state of the action
            // objects.
            // We could extend this to be done on each action object in turn
            // as well.
            if (updater != null)
                updater.updateItems();
    ...
    {
        return new MenuItem(m_Menu, SWT.SEPARATOR);
    }

    public BaseMenu addSubmenu(String name)
    {
        return new BaseMenu(m_Menu, name);
    }
    ...
    public MenuItem addCheckedItem(final AbstractAction action,
