...
public class KernelMenu
{
    ...
    public void updateMenu()
    {
        ...
        m_StopKernel.setEnabled(m_Document.isConnected()
                && !m_Document.isRemote());
        m_RemoteConnect.setEnabled(!(m_Document.isConnected() && m_Document
                .isRemote()));
    ...
    private void remoteConnectPerformed(ActionEvent e)
    {
        if (m_Document.isConnected())
        {
            int result = m_Frame
                    .ShowMessageBox(
                            "Kernel running",
                            "You are currently running a local Soar kernel.\nThis needs to be shutdown before connecting to a remote Soar.\n\nWould you like to shutdown the local kernel now?",
        ...
        RemoteDialog.RemoteInfo ip = RemoteDialog.showDialog(m_Frame,
                "Remote Soar Kernel Connection");
