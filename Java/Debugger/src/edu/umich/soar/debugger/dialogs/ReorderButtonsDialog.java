...
public class ReorderButtonsDialog extends BaseDialog
{
     ...
     *            The list of buttons (described as ButtonInfo objects) to
     *            re-order
     * @return A new sequence of buttonInfo objects or null if they cancelled
     *         the dialog
     ********************************************************************************************/
    public static ArrayList<ButtonView.ButtonInfo> showDialog(Composite parent,
            String title, ArrayList<ButtonView.ButtonInfo> buttonInfoList)
    {
        // Create the dialog window
        ReorderButtonsDialog dialog = new ReorderButtonsDialog(parent, title,
                buttonInfoList);
            ...
            ArrayList<ButtonView.ButtonInfo> buttonInfoList)
    {
        ...
        m_ButtonListControl = new List(right, SWT.SINGLE | SWT.V_SCROLL
                | SWT.BORDER);
    ...
    protected void pressed(boolean up, boolean toLimit)
    {
        ...
        if (selection == -1)
        {
            MainFrame.ShowMessageBox(getDialog().getShell(), "Error",
                    "Select a button first before trying to move it.", SWT.OK);
            return;
        }
        ...
        if (up && toLimit)
            dest = 0;
        if (up && !toLimit && selection > 0)
            dest = selection - 1;
        if (!up && toLimit)
            dest = limit;
        if (!up && !toLimit && selection < limit)
            dest = selection + 1;
