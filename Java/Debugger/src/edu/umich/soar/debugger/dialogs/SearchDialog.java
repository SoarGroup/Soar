...
public class SearchDialog extends BaseDialog
{
     ...
     ********************************************************************************************/
    public static SearchDialog showDialog(MainFrame frame, String title,
            AbstractView view)
    {
    ...
    protected void createContents(final Composite window)
    {
        ...
        // Set the initial values for the controls based on the users previous
        // choices
        // or the defaults defined here.
        m_Down.setSelection(m_Frame.getAppBooleanProperty("Search.Down", true));
        m_Match.setSelection(m_Frame.getAppBooleanProperty("Search.MatchCase",
                false));
        m_Wrap.setSelection(m_Frame.getAppBooleanProperty("Search.Wrap", true));
        m_SearchHidden.setSelection(m_Frame.getAppBooleanProperty(
                "Search.Hidden", true));
        m_KeepWindow.setSelection(m_Frame.getAppBooleanProperty("Search.Keep",
                true));
        ...
        // Registering a filter means it will apply to any keydown within the
        // dialog's display
        // rather than having to register with each specific control in the
        // dialog
        // (which would otherwise be necessary as the key strokes only go to the
        // control with focus).
    ...
    public void find()
    {
        boolean found = m_View.find(m_FindText.getText(),
                m_Down.getSelection(), m_Match.getSelection(), m_Wrap
                        .getSelection(), m_SearchHidden.getSelection());
