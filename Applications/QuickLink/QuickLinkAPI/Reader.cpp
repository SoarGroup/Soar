...
Reader::ReadMe(istream* in)
{
	...
	{
		QL->locFinder(in);  //gets location of file  //QLInterface
		ofstream tempFile;
	...
	{
		*in >> QL->second >> QL->third >> QL->fourth;  //used to make parser changes easier  //QLInterface
		QL->parent = QL->second;
	...
	{
		*in >> QL->second >> QL->third >> QL->fourth >> QL->fifth;  //QLInterface
		QL->parent = QL->second;
		QL->makeUpper(QL->parent);
		QL->path = QL->third.substr(1, QL->third.size()-1);  //gets rid of ^
		QL->OldVal = QL->fourth;
	...
	{
		*in >> QL->second >> QL->third >> QL->fourth;  //QLInterface
		QL->parent = QL->second;
		QL->makeUpper(QL->parent);
		QL->path = QL->third.substr(1,QL->third.size()-1);  //gets rid of ^
	...
	{
		QL->PurgeAllVectors();  //QLInterface
		QL->promptToSave();
		QL->pKernel->Shutdown();
		delete QL->pKernel;
		return "***QUITRETURN***";
	...
	{
		QL->shouldPrintWM = false; //we do not want to print WM everytime a new structure is added  //QLInterface
	...
	{
		if(QL->fileStack.size() == 0)    //a process cannot be running  //QLInterface
	...
	else if(QL->first == "LASTINPUT" || QL->first == "LI")
	{
		...
		toReturn = "***VOIDRETURN***";  //QLInterface
		return toReturn;
	}
	else if(QL->first == "COMMIT")
	{
		if(QL->pKernel->IsAgentValid(QL->pAgent))  //QLInterface
			QL->pAgent->Commit();
		else
	...
	{
		char cmd[1000] ;  //used to get entire line including spaces  //QLInterface
		in->getline(cmd,1000) ;
		string strcmd = cmd;
	...
	{
		cout << endl << "***Structures will now be printed in Tree-form***" << endl;  //QLInterface
		QL->printTree = true;
		toReturn = QL->first;
		return toReturn;
	}
	...
	{
		cout << endl << "***Structures will now be printed in Soar-form***" << endl;  //QLInterface
		QL->printTree = false;
		toReturn = QL->first;
		return toReturn;
	}
	...
	{
		QL->shouldPrintWM = true;    //QLInterface
	...
	{
		QL->spawnDebug();  //QLInterface
		toReturn = "***VOIDRETURN***";
		return toReturn;
	}
	else if( QL->first == "LISTEN" )
		Listener hear( QL->pKernel , QL->pAgent );  //QLInterface
	else 
	{
