...
This occurs when a subgoal creates a local structure, which it then links to a
superstate as a result. Things added to this structure are also results, as
they are linked to the superstate. However, if that substructure is created
with rules that match the local state only, Soar cannot build a chunk for the
