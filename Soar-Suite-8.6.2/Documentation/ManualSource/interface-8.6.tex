...
\begin{tabular}{|l|p{12cm}|}
\hline 
\soar{ 0 } & watch nothing; equivalent to \^a€“N  \\
\hline
...
\subsection{\soarb{clog}}
 ...
 \subsubsection*{Description}
  ...
  Invoke \soar{clog}   with no arguments (or with \soar{-q} ) to query the current logging status. Pass a filename to start logging to that file (relative to the command line interface's home directory (see the home command)). Use the \soar{close}   option to stop logging. 
 \subsubsection*{Examples}
...
\hline 
\soar{ -d, --disable,}\soar{--off } & Remove the named alias.  \\
\hline
...
\subsubsection*{Description}
 ...
 procedure checks to see if the name already exists, and does not destroy existing procedures or aliases by the same name. Existing aliases can be removed by using the \soar{-d}
 flag. With no arguments, \soar{alias}
