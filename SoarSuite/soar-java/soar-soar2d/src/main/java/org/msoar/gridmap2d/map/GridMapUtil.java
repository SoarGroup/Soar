...
import org.msoar.gridmap2d.map.RoomMap.RoomObjectInfo;
...
public class GridMapUtil {
	...
	static boolean generateRoomStructure(GridMapData data, RoomMapBuildData roomData) {
		...
		for (location[1] = 1; location[1] < (data.cells.size() - 1); ++location[1]) {
			for (location[0] = 1; location[0] < (data.cells.size() - 1); ++location[0]) {
				...
				if (cell.hasAnyWithProperty(org.msoar.gridmap2d.Names.kPropertyBlock) || cell.hasObject(org.msoar.gridmap2d.Names.kPropertyGateway)) {
					logger.trace("not a room candidate");
					continue;
				}
				
				assert cell.getObject(org.msoar.gridmap2d.Names.kRoomID) == null;
				...
				CellObject roomObject = data.cellObjectManager.createObject(org.msoar.gridmap2d.Names.kRoomID);
				roomObject.setProperty(org.msoar.gridmap2d.Names.kPropertyNumber, Integer.toString(roomNumber));
				...
				while (true) {
					...
					CellObject gatewayObject = cell.getObject(org.msoar.gridmap2d.Names.kGatewayID);
					CellObject wallObject = cell.getObject(org.msoar.gridmap2d.Names.kWallID);
		...
		for (RoomObjectInfo info : roomData.roomObjectInfoMap.values()) {
			info.area = data.cells.getCell(info.location).getAllWithProperty(org.msoar.gridmap2d.Names.kPropertyNumber).get(0).getIntProperty(org.msoar.gridmap2d.Names.kPropertyNumber, -1);
		}
	...
	private static void gatewaysToAreasStep(GridMapData data, List<Barrier> gatewayBarriers, RoomMapBuildData roomData) {
		...
		while (iter.hasNext()) {
			...
			// duplicates exist in this list, check to see if we're already a room
			{
				...
				if (cell.hasObject(org.msoar.gridmap2d.Names.kRoomID)) {
					// we have already processed this room, just need to add the room id
					// to the gateway's destination list
					RoomMapBuildData.addDestinationToGateway(roomData, cell.getObject(org.msoar.gridmap2d.Names.kRoomID).getIntProperty(org.msoar.gridmap2d.Names.kPropertyNumber, -1), gatewayBarrier.id);
					continue;
				}
			}
			...
			CellObject theNewRoomObject = data.cellObjectManager.createObject(org.msoar.gridmap2d.Names.kRoomID);
			theNewRoomObject.setProperty(org.msoar.gridmap2d.Names.kPropertyNumber, Integer.toString(roomNumber));
			{
				Cell cell = data.cells.getCell(gatewayBarrier.left);
			...
			if (incrementDirection == Direction.NONE) {
				...
				if (cell.getObject(org.msoar.gridmap2d.Names.kWallID) != null) {
					// horizontal gateway
					incrementDirection = Direction.EAST;
				} else {
					// vertical gateway
	...
	private static void doNewGateway(RoomMapBuildData roomData, GridMapData data, int [] startPoint, int [] endPoint, Direction direction, Direction walkDirection, int roomNumber, List<Barrier> barrierList) {
		...
		while (true) {
			...
			CellObject gatewayObject = data.cellObjectManager.createObject(org.msoar.gridmap2d.Names.kGatewayID);
			gatewayObject.removeProperty(org.msoar.gridmap2d.Names.kPropertyGatewayRender);
			...
			gatewayDestinations.add(new Integer(roomNumber));
			gatewayDestinations.add(new Integer(cell.getObject(org.msoar.gridmap2d.Names.kRoomID).getIntProperty(org.msoar.gridmap2d.Names.kPropertyNumber, -1)));
			roomData.gatewayDestinationMap.put(new Integer(currentBarrier.id), gatewayDestinations);
