 ...
 * @author doug
 *
 * Represents an eater in a way that can interface with Tosca
 */
 ...
 * @author doug
 *
 * the Tosca eater class is a human eater except controlled by tosca instead of
 * human input.  someday may share code with eater so we're extending that for
 * now.
 * 
 * The Tosca eater is based on Soar Eater.
 * 
 */
public class ToscaEater extends Eater {
	 ...
	 * the 5x5 vision grid
	 */
	private ToscaCell[][] cells = new ToscaCell[(Soar2D.config.kEaterVision * 2 ) + 1][(Soar2D.config.kEaterVision * 2 ) + 1];
	/**
	 ...
	 */
	public ToscaEater(Agent agent, PlayerConfig playerConfig) {
		super(playerConfig);
		this.agent = agent;
	...
	public void update(World world, java.awt.Point location) {
		...
		for (int x = 0; x < cells.length; ++x) {
			...
			for (int y = 0; y < cells[x].length; ++y) {
				...
				// get the current ToscaCell to update
				ToscaCell ToscaCell = cells[x][y];
				...
				if (moved) {
					...
					destroyWMEsAndClear(ToscaCell.content);
					if (ToscaCell.box != null) {
						agent.DestroyWME(ToscaCell.box);
						ToscaCell.box = null;
						ToscaCell.boxProperties.clear();
					}
					
					// If we're out of bounds create a wall
					if (cell == null) {
						createContent(ToscaCell.content, ToscaCell, Names.kWallID);
						continue;
					}
					...
					if (!cell.enterable()) {
						...
						// update the ToscaCell with the new content
						// use the id property as its id on the input link
						assert wall.hasProperty(Names.kPropertyID);
						createContent(ToscaCell.content, ToscaCell, wall.getProperty(Names.kPropertyID));
						continue;
					}
					...
					if (player != null) {
						...
						createContent(ToscaCell.content, ToscaCell, Names.kEaterID);
					}
					...
					while (objectIter.hasNext()) {
						...
						if (cell.hasObject(food.getName())) {
							...
							createContent(ToscaCell.content, ToscaCell, food.getProperty(Names.kPropertyID));
						}
					}
					...
					if (boxes.size() > 0) {
						...
						createBox(ToscaCell, boxes.get(0));
					...
					if(!hadFood && (player == null) && (boxes.size() == 0)) {
						createContent(ToscaCell.content, ToscaCell, Names.kEmpty);
					}
				} else {
					...
					//   if it doesn't exist in the ToscaCell map
					//     create it and store in new map
					//   else
					//     take existing and store in new map
					//   remove from ToscaCell map
					// destroy remaining elements in ToscaCell map
					// assign new map to ToscaCell map
					...
					if (player != null) {
						...
						StringElement element = ToscaCell.content.remove(Names.kEaterID);
						if (element == null) {
							// no, create it
							createContent(newContent, ToscaCell, Names.kEaterID);
						} else {
							// yes, keep it
					...
					while (objectIter.hasNext()) {
						...
						if (cell.hasObject(food.getName())) {
							...
							StringElement element = ToscaCell.content.remove(food.getName());
							if (element == null) {
								// no, create it
								assert food.hasProperty(Names.kPropertyID);
								createContent(newContent, ToscaCell, food.getProperty(Names.kPropertyID));
							} else {
								// yes, save it
								assert food.hasProperty(Names.kPropertyID);
					...
					if (boxes.size() > 0) {
						...
						if (ToscaCell.box == null) {
							...
							assert ToscaCell.boxProperties.size() == 0;
							
							// and create one
							createBox(ToscaCell, box);
						} else {
							...
							while (iter.hasNext()) {
								...
								StringElement element = ToscaCell.boxProperties.remove(name);
								if (element == null) {
									// no, add it
									String value = box.getProperty(name);
									element = agent.CreateStringWME(ToscaCell.box, name, value);
								}
								// save it regardless
							...
							destroyWMEsAndClear(ToscaCell.boxProperties);
							// set the new ones
							ToscaCell.boxProperties = newBoxProperties;
						}
					}
					...
					// wall is implied since we can't get here if there is a wall
					if(!hadFood && (player == null) && (boxes.size() == 0)) {
						StringElement element = ToscaCell.content.remove(Names.kEmpty);
						if (element == null) {
							createContent(newContent, ToscaCell, Names.kEmpty);
						} else {
	...
	/**
	 * @param ToscaCell the cell that contains the box
	 * @param box the box object
	 */
	private void createBox(ToscaCell ToscaCell, CellObject box) {
		// create the wme
		assert box.hasProperty(Names.kPropertyID);
		ToscaCell.box = agent.CreateIdWME(ToscaCell.me, box.getProperty(Names.kPropertyID));
		// make sure that happened
		assert ToscaCell.box != null;
		...
		while (iter.hasNext()) {
			String name = iter.next();
			String value = box.getProperty(name);
			StringElement element = agent.CreateStringWME(ToscaCell.box, name, value);
			ToscaCell.boxProperties.put(name, element);
		}
	}
	 ...
	 * @param ToscaCell the cell
	 * @param name the name of the content to create
	 */
	private void createContent(HashMap<String, StringElement> map, ToscaCell ToscaCell, String name) {
		// create the wme
		StringElement element = agent.CreateStringWME(ToscaCell.me, Names.kContentID, name);
