...
#
#	last update: 6.15.00 Acar Altinsel
#
...
proc drawscreen {w} {
	...
	checkbutton $w.speed -text "No Animation" -variable noanim	
	button $w.ok -text Close -command "dismiss $w"
       
	pack $w.msg -side top
	pack $w.frame -side top -padx 20
	pack $w.solve $w.stop $w.init $w.exch $w.speed -side left -padx 1m -pady 2m
...
proc drawstate {ml cl mr cr b} {
	...
	if {$ml >= 1} {
		.mac.frame.can create image 90 375 -image missionary 
	}
	if {$ml >= 2} {
		.mac.frame.can create image 60 375 -image missionary 
	}
	if {$ml >= 3} {
		.mac.frame.can create image 30 375 -image missionary 
	}
	
	if {$cl >= 1} {
		.mac.frame.can create image 170 310 -image cannibal 
	}
	if {$cl >= 2} {
		.mac.frame.can create image 130 310 -image cannibal 
	}
	if {$cl >= 3} {
		.mac.frame.can create image 90 310 -image cannibal 
	}
	
	if {$b == "l"} {
		.mac.frame.can create image 200 365 -image boat 
	}
	
	
	if {$mr >= 1} {
		.mac.frame.can create image 540 375 -image missionary 
	}
	if {$mr >= 2} {
		.mac.frame.can create image 570 375 -image missionary 
	}
	if {$mr >= 3} {
		.mac.frame.can create image 600 375 -image missionary 
	}
	
	if {$cr >= 1} {
		.mac.frame.can create image 490 310 -image cannibal 
	}
	if {$cr >= 2} {
		.mac.frame.can create image 530 310 -image cannibal 
	}
	if {$cr >= 3} {
		.mac.frame.can create image 570 310 -image cannibal 
	}
	
	if {$b == "r"} {
		.mac.frame.can create image 420 365 -image boat 
	}
