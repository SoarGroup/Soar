...
// bool adding, float &res, bool &changed) {
	float newres;
	const sgnode *a, *b;
	int axis = -1;
	double top, bot;
...
filter *make_passthru_filter(Symbol *root, soar_interface *si, scene *scn, filter_input *input) {
    return new passthru_filter(root, si, input);
}
