...
def kill_match(tournament_name):
	message = {}
...
def check_productions(zip_data, primary_file):
	...
	try:
		...
		if not os.path.isfile(tank_source_file_name):
			raise ValueError("Primary file points to a directory, should be a .soar file")
...
def soar_test():
	kernel = Python_sml_ClientInterface.Kernel.CreateKernelInCurrentThread()
	if kernel == None:
		return "Kernel didn't create"
	agent = kernel.CreateAgent('Soar1')
	if agent == None:
		return "Agent didn't create"
	kernel.shutdown()
	del kernel
	return None
