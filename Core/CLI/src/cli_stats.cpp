...
bool CommandLineInterface::DoStats(const StatsBitset& options, int sort)
{
    ...
    if (options.test(STATS_CYCLE))
    {
        ...
        switch (sort)
        {
            ...
            case 0: // default sort
            case 1: // sort dc
                select = desc ? thisAgent->stats_stmts->sel_dc_dec : thisAgent->stats_stmts->sel_dc_inc;
                break;
            case 2: // sort time
                select = desc ? thisAgent->stats_stmts->sel_time_dec : thisAgent->stats_stmts->sel_time_inc;
                break;
            case 3: // sort wm_changes
                select = desc ? thisAgent->stats_stmts->sel_wm_changes_dec : thisAgent->stats_stmts->sel_wm_changes_inc;
                break;
            case 4: // sort firing_count
                select = desc ? thisAgent->stats_stmts->sel_firing_count_dec : thisAgent->stats_stmts->sel_firing_count_inc;
                break;
        }
    ...
    std::string temp;
    AppendArgTagFast(sml_Names::kParamStatsProductionCountDefault,                sml_Names::kTypeInt,    to_string(thisAgent->num_productions_of_type[DEFAULT_PRODUCTION_TYPE], temp));
    ...
    AppendArgTagFast(sml_Names::kParamStatsWmeCountAddition,                    sml_Names::kTypeInt,    to_string(thisAgent->wme_addition_count, temp));
    AppendArgTagFast(sml_Names::kParamStatsWmeCountRemoval,                        sml_Names::kTypeInt,    to_string(thisAgent->wme_removal_count, temp));
    AppendArgTagFast(sml_Names::kParamStatsWmeCount,                            sml_Names::kTypeInt,    to_string(thisAgent->num_wmes_in_rete, temp));
    AppendArgTagFast(sml_Names::kParamStatsWmeCountAverage,                        sml_Names::kTypeDouble, to_string((thisAgent->num_wm_sizes_accumulated ? (thisAgent->cumulative_wm_size / thisAgent->num_wm_sizes_accumulated) : 0.0), temp));
    AppendArgTagFast(sml_Names::kParamStatsWmeCountMax,                            sml_Names::kTypeInt,    to_string(thisAgent->max_wm_size, temp));
    ...
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleTimeCycle,            sml_Names::kTypeInt,    to_string(thisAgent->max_dc_time_cycle, temp));
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleTimeValueSec,        sml_Names::kTypeDouble,    to_string(thisAgent->max_dc_time_usec / 100000.0, temp));
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleTimeValueUSec,       sml_Names::kTypeInt,    to_string(thisAgent->max_dc_time_usec, temp));
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleEpMemTimeCycle,      sml_Names::kTypeInt,    to_string(thisAgent->max_dc_epmem_time_cycle, temp));
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleEpMemTimeValueSec,   sml_Names::kTypeDouble,    to_string(thisAgent->max_dc_epmem_time_sec, temp));
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleSMemTimeCycle,       sml_Names::kTypeInt,    to_string(thisAgent->max_dc_smem_time_cycle, temp));
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleSMemTimeValueSec,    sml_Names::kTypeDouble,    to_string(thisAgent->max_dc_smem_time_sec, temp));
#endif // NO_TIMING_STUFF
    
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleWMChangesCycle,        sml_Names::kTypeInt,    to_string(thisAgent->max_dc_wm_changes_cycle, temp));
    AppendArgTagFast(sml_Names::kParamStatsMaxDecisionCycleWMChangesValue,        sml_Names::kTypeInt,    to_string(thisAgent->max_dc_wm_changes_value, temp));
    ...
    AppendArgTagFast(sml_Names::kParamStatsMemoryUsageMiscellaneous,            sml_Names::kTypeInt,    to_string(thisAgent->memory_for_usage[MISCELLANEOUS_MEM_USAGE], temp));
    AppendArgTagFast(sml_Names::kParamStatsMemoryUsageHash,                        sml_Names::kTypeInt,    to_string(thisAgent->memory_for_usage[HASH_TABLE_MEM_USAGE], temp));
    AppendArgTagFast(sml_Names::kParamStatsMemoryUsageString,                    sml_Names::kTypeInt,    to_string(thisAgent->memory_for_usage[STRING_MEM_USAGE], temp));
    AppendArgTagFast(sml_Names::kParamStatsMemoryUsagePool,                        sml_Names::kTypeInt,    to_string(thisAgent->memory_for_usage[POOL_MEM_USAGE], temp));
    AppendArgTagFast(sml_Names::kParamStatsMemoryUsageStatsOverhead,            sml_Names::kTypeInt,    to_string(thisAgent->memory_for_usage[STATS_OVERHEAD_MEM_USAGE], temp));
...
void CommandLineInterface::GetMemoryStats()
{
    ...
    m_Result << std::setw(8) << thisAgent->memory_for_usage[STATS_OVERHEAD_MEM_USAGE] << " bytes statistics overhead\n";
    m_Result << std::setw(8) << thisAgent->memory_for_usage[STRING_MEM_USAGE] << " bytes for strings\n";
    m_Result << std::setw(8) << thisAgent->memory_for_usage[HASH_TABLE_MEM_USAGE] << " bytes for hash tables\n";
    m_Result << std::setw(8) << thisAgent->memory_for_usage[POOL_MEM_USAGE] << " bytes for various memory pools\n";
    m_Result << std::setw(8) << thisAgent->memory_for_usage[MISCELLANEOUS_MEM_USAGE] << " bytes for miscellaneous other things\n";
...
void CommandLineInterface::GetReteStats()
{
    ...
    m_Result << "Token additions: " << thisAgent->token_additions << "   If no sharing: " << thisAgent->token_additions_without_sharing << "\n";
#endif
