...
bool CommandLineInterface::DoSMem(const char pOp, const std::string* pArg1, const std::string* pArg2, const std::string* pArg3)
{
    ...
    else if (pOp == 's')
    {
        ...
        if (!strcmp(pArg1->c_str(), "spreading") && !(thisAgent->SMem->settings->activation_mode->get_value() == smem_param_container::act_base || thisAgent->SMem->settings->activation_mode->get_value() == smem_param_container::act_none))
        {
            return SetError("Spreading activation cannot be turned on until the 'activation-mode' is also set to base-level or none.\n"
                    "Run 'smem --set activation-mode base-level' first if you intend to use spreading.");
        }
        if (!strcmp(pArg1->c_str(), "activation-mode") && (thisAgent->SMem->settings->activation_mode->get_value() == smem_param_container::act_base || thisAgent->SMem->settings->activation_mode->get_value() == smem_param_container::act_none) && thisAgent->SMem->settings->spreading->get_value() == on)
        {
