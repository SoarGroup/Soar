...
//        PrintCLIMessage_Item("mirroring:", thisAgent->smem_params->mirroring, 40);
		PrintCLIMessage_Justify("mirroring:", smem->is_mirroring() ? "true" : "false", 40);
		PrintCLIMessage_Justify("recursive storage:", smem->is_storing_recursively() ? "true" : "false", 40);
        PrintCLIMessage("");
    ...
    {
        std::string err;
		bool result = smem->parse_add_command(thisAgent, *pAttr, &err);
...
//    else if (pOp == 'b')
//    {
//        std::string err = "Feature is not implemented.";
//		bool result = false; //smem_backup_db(thisAgent, pAttr->c_str(), &(err));
//        
//        if (!result)
//        {
...
//    }
    else if (pOp == 'p')
    {
        ...
        if (pAttr)
        {
            get_lexeme_from_string(thisAgent, pAttr->c_str());
            if (thisAgent->lexeme.type == IDENTIFIER_LEXEME)
				sym = smem->retrieve_lti(thisAgent, thisAgent->lexeme.id_letter, thisAgent->lexeme.id_number, &result);
        ...
        if (!sym)
        {
			smem->print_memory(thisAgent, &viz);
			
			if (!viz.empty())
                PrintCLIMessage_Header("Semantic Memory", 40);
        }
        else
			smem->print_lti(thisAgent, sym, &viz, depth);
