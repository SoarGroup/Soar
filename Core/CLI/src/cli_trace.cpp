...
bool CommandLineInterface::DoTraceBackwardsCompatible(std::vector< std::string >& argv, bool fromTraceLevel)
{
    if (fromTraceLevel)
    {
        return m_Parser.handle_command(argv);
    } else {
        return m_Parser.handle_command(argv);
    }
}
...
bool CommandLineInterface::DoTrace(const WatchBitset& options, const WatchBitset& settings, const int wmeSetting, const int learnSetting)
{
    ...
    if (options.none())
    {
        ...
        if (m_RawOutput)
        {
            m_Result << "Current watch settings:\n  Decisions:  " << (thisAgent->trace_settings[TRACE_CONTEXT_DECISIONS_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Phases:  " << (thisAgent->trace_settings[TRACE_PHASES_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Default productions:  " << (thisAgent->trace_settings[TRACE_FIRINGS_OF_DEFAULT_PRODS_SYSPARAM] ? "on" : "off");
            m_Result << "\n  User productions:  " << (thisAgent->trace_settings[TRACE_FIRINGS_OF_USER_PRODS_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Chunks:  " << (thisAgent->trace_settings[TRACE_FIRINGS_OF_CHUNKS_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Justifications:  " << (thisAgent->trace_settings[TRACE_FIRINGS_OF_JUSTIFICATIONS_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Templates:  " << (thisAgent->trace_settings[TRACE_FIRINGS_OF_TEMPLATES_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Working memory changes:  " << (thisAgent->trace_settings[TRACE_WM_CHANGES_SYSPARAM] ? "on" : "off");
            m_Result << "\n  WME detail level:  ";
            
            switch (thisAgent->trace_settings[TRACE_FIRINGS_WME_TRACE_TYPE_SYSPARAM])
            {
            ...
            m_Result << "\n  Preferences generated by firings/retractions:  " << (thisAgent->trace_settings[TRACE_FIRINGS_PREFERENCES_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Learning:  ";
            ...
            m_Result << "\n  Backtracing:  " << (thisAgent->trace_settings[TRACE_BACKTRACING_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Indifferent selection:  " << (thisAgent->trace_settings[TRACE_INDIFFERENT_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Soar-RL:  " << (thisAgent->trace_settings[TRACE_RL_SYSPARAM] ? "on" : "off");
            m_Result << "\n  Waterfall:  " << (thisAgent->trace_settings[TRACE_WATERFALL_SYSPARAM] ? "on" : "off");
            m_Result << "\n  EpMem:  " << (thisAgent->trace_settings[TRACE_EPMEM_SYSPARAM] ? "on" : "off");
            m_Result << "\n  SMem:  " << (thisAgent->trace_settings[TRACE_SMEM_SYSPARAM] ? "on" : "off");
            m_Result << "\n  WMA:  " << (thisAgent->trace_settings[TRACE_WMA_SYSPARAM] ? "on" : "off");
            m_Result << "\n  GDS WME changes:  " << (thisAgent->trace_settings[TRACE_GDS_WMES_SYSPARAM] ? "on" : "off");
            m_Result << "\n  GDS state removals:  " << (thisAgent->trace_settings[TRACE_GDS_STATE_REMOVAL_SYSPARAM] ? "on" : "off");
