...
bool KernelSML::HandleCreateAgent(AgentSML* pAgentSML, char const* pCommandName, Connection* pConnection, AnalyzeXML* pIncoming, soarxml::ElementXML* pResponse)
{
    ...
    if (!directory.empty())
    {
        ...
        if (!lResult.empty())
        {
            return InvalidArg(pConnection, pResponse, pCommandName, lResult.c_str()) ;
        }
    } else {
        return InvalidArg(pConnection, pResponse, pCommandName, "Could not find settings.soar file.");
    }
