...
/* Windows does not like the memory pool allocators for these two list types for
 * some reason. Not sure why yet, so using without  custom allocator for now. */
#if defined(USE_MEM_POOL_ALLOCATORS) && !defined(_WIN32)
