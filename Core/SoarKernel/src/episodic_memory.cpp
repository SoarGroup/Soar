 ...
 **************************************************************************/
epmem_timer *epmem_add_timer( const char *name, const long parent )
{
	// new timer entry
...
void epmem_start_timer( agent *my_agent, const long timer )
{
	if ( epmem_valid_timer( my_agent, timer ) )
	{
		...
		if ( my_agent->epmem_timers[ timer ]->parent != NULL )
			epmem_start_timer( my_agent, my_agent->epmem_timers[ timer ]->parent );
	}
}
...
void epmem_stop_timer( agent *my_agent, const long timer )
{
	if ( epmem_valid_timer( my_agent, timer ) )
	{
		...
		if ( my_agent->epmem_timers[ timer ]->parent != NULL )
			epmem_stop_timer( my_agent, my_agent->epmem_timers[ timer ]->parent );
	}
}
...
void epmem_init_db( agent *my_agent )
{
	...
	else
	{
		...
		{			
			...
			// provenance specific stuff
			if ( provenance == EPMEM_PROVENANCE_ON )
			{
			}
			else if ( provenance == EPMEM_PROVENANCE_OFF )
			{
			}
		}
		// INIT::BIGTREE_RANGE
		...
		else if ( indexing == EPMEM_INDEXING_BIGTREE_HYBRID )
		{
			...
			// custom statement for inserting episodes
			sqlite3_prepare_v2( my_agent->epmem_db, "INSERT INTO episodes (id,start,end) VALUES (?,?,?)", -1, &( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_ADD_NODE ] ), &tail );

			//
			
			// images table
			sqlite3_prepare_v2( my_agent->epmem_db, "CREATE TABLE IF NOT EXISTS images (time INTEGER PRIMARY KEY, ids BLOB)", -1, &create, &tail );
			sqlite3_step( create );					
			sqlite3_finalize( create );

			// custom statement for inserting images
			sqlite3_prepare_v2( my_agent->epmem_db, "INSERT INTO images (time,ids) VALUES (?,?)", -1, &( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_ADD_EPISODE ] ), &tail );
			...
			sqlite3_prepare_v2( my_agent->epmem_db, "SELECT ids FROM images WHERE time=?", -1, &( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_GET_EPISODE ] ), &tail );

			//

			// custom statement for validating an episode
			sqlite3_prepare_v2( my_agent->epmem_db, "SELECT COUNT(*) AS ct FROM images WHERE time=?", -1, &( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_VALID_EPISODE ] ), &tail );

			// custom statement for finding the next episode
			sqlite3_prepare_v2( my_agent->epmem_db, "SELECT time FROM images WHERE time>? ORDER BY time ASC LIMIT 1", -1, &( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_NEXT_EPISODE ] ), &tail );

			// custom statement for finding the previous episode
			sqlite3_prepare_v2( my_agent->epmem_db, "SELECT time FROM images WHERE time<? ORDER BY time DESC LIMIT 1", -1, &( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_PREV_EPISODE ] ), &tail );

			//

			// get max time
			sqlite3_prepare_v2( my_agent->epmem_db, "SELECT MAX(time) FROM images", -1, &create, &tail );
			if ( sqlite3_step( create ) == SQLITE_ROW )
				epmem_set_stat( my_agent, (const long) EPMEM_STAT_TIME, ( sqlite3_column_int64( create, 0 ) + 1 ) );
			sqlite3_finalize( create );
			time_max = epmem_get_stat( my_agent, (const long) EPMEM_STAT_TIME );
...
void epmem_consider_new_episode( agent *my_agent )
{
	...
	//////////////////////////////////////////////////////////////////////////
	epmem_stop_timer( my_agent, EPMEM_TIMER_STORAGE_CONSIDER );
	//////////////////////////////////////////////////////////////////////////
...
{		
	...
	// NEW::BIGTREE_INSTANCE
	if ( indexing == EPMEM_INDEXING_BIGTREE_INSTANCE )
	{
		//////////////////////////////////////////////////////////////////////////
		epmem_start_timer( my_agent, EPMEM_TIMER_STORAGE_COLLECT );
		//////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////////
		epmem_stop_timer( my_agent, EPMEM_TIMER_STORAGE_COLLECT );
		//////////////////////////////////////////////////////////////////////////

		//////////////////////////////////////////////////////////////////////////
		epmem_start_timer( my_agent, EPMEM_TIMER_STORAGE_SAVE );
		//////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////////
		epmem_stop_timer( my_agent, EPMEM_TIMER_STORAGE_SAVE );
		//////////////////////////////////////////////////////////////////////////
	}
	// NEW::BIGTREE_RANGE
	else if ( indexing == EPMEM_INDEXING_BIGTREE_RANGE )
...
{
	//////////////////////////////////////////////////////////////////////////
	epmem_start_timer( my_agent, EPMEM_TIMER_INSTALL_VALID );
	//////////////////////////////////////////////////////////////////////////
	
	const long indexing = epmem_get_parameter( my_agent, EPMEM_PARAM_INDEXING, EPMEM_RETURN_LONG );
	bool return_val = false;

	// VALID::BIGTREE_INSTANCE
	if ( indexing == EPMEM_INDEXING_BIGTREE_INSTANCE )
	{		
	...
	//////////////////////////////////////////////////////////////////////////
	epmem_stop_timer( my_agent, EPMEM_TIMER_INSTALL_VALID );
	//////////////////////////////////////////////////////////////////////////
...
void epmem_process_query( agent *my_agent, Symbol *state, Symbol *query, Symbol *neg_query, std::vector<epmem_time_id> *prohibit, epmem_time_id before, epmem_time_id after )
{
	...
	if ( ( len_query != 0 ) || ( len_neg_query != 0 ) )
	{
		...
		if ( indexing == EPMEM_INDEXING_BIGTREE_INSTANCE )
		{
			...
			{				
				sqlite3_stmt *search;
				unsigned long long cue_size;
				epmem_time_id result_time;
				unsigned long long result_cardinality;
				...
				if ( leaf_ids[ EPMEM_NODE_POS ].empty() || leaf_ids[ EPMEM_NODE_NEG ].empty() )
				{
					int index = ( ( leaf_ids[ EPMEM_NODE_POS ].empty() )?( EPMEM_NODE_NEG ):( EPMEM_NODE_POS ) );
					const char *sort_dir = ( ( leaf_ids[ EPMEM_NODE_POS ].empty() )?( "ASC" ):( "DESC" ) );
					...
					// bind variables
					int var = 1;
					sqlite3_bind_int64( search, var++, ( ( leaf_ids[ EPMEM_NODE_POS ].empty() )?( -1 ):( 1 ) ) );
					sqlite3_bind_int64( search, var++, ( ( leaf_ids[ EPMEM_NODE_POS ].empty() )?( -1 ):( 1 ) ) );
				...
				else
				{
					...
					// positive
					leaf_p = leaf_ids[ EPMEM_NODE_POS ].begin();
					while ( leaf_p != leaf_ids[ EPMEM_NODE_POS ].end() )
					{
						sqlite3_bind_int64( search, var++, (*leaf_p) );
						leaf_p++;
					}
					...
					// neg
					leaf_p = leaf_ids[ EPMEM_NODE_NEG ].begin();
					while ( leaf_p != leaf_ids[ EPMEM_NODE_NEG ].end() )
					{
						sqlite3_bind_int64( search, var++, (*leaf_p) );
						leaf_p++;
					}					
					...
					// meta data
					cue_size = ( leaf_ids[ EPMEM_NODE_POS ].size() + leaf_ids[ EPMEM_NODE_NEG ].size() );
				}
				//////////////////////////////////////////////////////////////////////////
				...
				if ( epmem_exec_query( my_agent, search, EPMEM_TIMER_QUERY_EXEC ) == SQLITE_ROW )				
				{
					//////////////////////////////////////////////////////////////////////////
					epmem_start_timer( my_agent, EPMEM_TIMER_QUERY_SEARCH );
					//////////////////////////////////////////////////////////////////////////
					...
					//////////////////////////////////////////////////////////////////////////
					epmem_stop_timer( my_agent, EPMEM_TIMER_QUERY_SEARCH );
					//////////////////////////////////////////////////////////////////////////
		...
		else if ( indexing == EPMEM_INDEXING_BIGTREE_RANGE )
		{
						...
						if ( (*wmes) != NULL )
						{
							for ( j=0; j<len; j++ )
							{
								...
								if ( (*wmes)[j]->value->common.symbol_type != IDENTIFIER_SYMBOL_TYPE )
								{
									...
									if ( sqlite3_step( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_FIND_ID ] ) == SQLITE_ROW )
										leaf_ids[i].push_back( epmem_create_leaf_node( sqlite3_column_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_FIND_ID ], 0 ), wma_get_wme_activation( my_agent, (*wmes)[j] ) ) );
			...
			{			
				...
				{				
					...
					{						
						sqlite3_bind_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_ADD_WEIGHT ], 1, (*leaf_p)->leaf_id );
						sqlite3_bind_double( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_ADD_WEIGHT ], 2, (*leaf_p)->leaf_weight );
			...
			{				
				...
				if ( !leaf_ids[ EPMEM_NODE_POS ].empty() )
				{
					//////////////////////////////////////////////////////////////////////////
					epmem_start_timer( my_agent, EPMEM_TIMER_QUERY_SQL );
					//////////////////////////////////////////////////////////////////////////
					...
					// perform insertion
					epmem_exec_query( my_agent, insert, EPMEM_TIMER_QUERY_EXEC );
					...
					do
					{
						temp_leaf = leaf_ids[0].front();
						leaf_ids[ EPMEM_NODE_POS ].pop_front();

						delete temp_leaf;

					} while ( !leaf_ids[ EPMEM_NODE_POS ].empty() );
				}
				...
				if ( !leaf_ids[ EPMEM_NODE_NEG ].empty() )
				{
					...
					do
					{
						temp_leaf = leaf_ids[ EPMEM_NODE_NEG ].front();
						leaf_ids[ EPMEM_NODE_NEG ].pop_front();

						delete temp_leaf;

					} while ( !leaf_ids[ EPMEM_NODE_NEG ].empty() );
				}
			}
			...
			// take care of NULL ends, add indexes
			{
				...
				epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_NULL_RANGES ], EPMEM_TIMER_QUERY_EXEC );				
				sqlite3_reset( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_NULL_RANGES ] );

				epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_RANGE_INDEX_1 ], EPMEM_TIMER_QUERY_EXEC );				
				sqlite3_reset( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_RANGE_INDEX_1 ] );

				epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_RANGE_INDEX_2 ], EPMEM_TIMER_QUERY_EXEC );
			...
			{			
				...
				while ( prohibit_p != prohibit->end() )
				{
					...
					else
					{
						...
						{
							epmem_time_id *range = new epmem_time_id[2];
							range[0] = start;
							range[1] = prev;
				...
				// remove ranges that are entirely contained within prohibit ranges
				{
					...
					// execute
					epmem_exec_query( my_agent, del, EPMEM_TIMER_QUERY_EXEC );					
				...
				// update ranges that are on the fringes of prohibition
				{
					...
					while ( pr_p != prohibit_ranges.end() )
					{
						...
						// execute+reset
						epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_DEL_PROHIB_LOW ], EPMEM_TIMER_QUERY_EXEC );
						epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_DEL_PROHIB_HIGH ], EPMEM_TIMER_QUERY_EXEC );									
				...
				// update ranges that contain prohibition
				{
					...
					{						
						...
						// execute+reset
						epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_DEL_PROHIB_CONTAIN ], EPMEM_TIMER_QUERY_EXEC );
						epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_DEL_PROHIB ], EPMEM_TIMER_QUERY_EXEC );						
			...
			// play king of the mountain (forward)
			{
				...
				// next pointers
				epmem_time_id low_id = EPMEM_MEMID_NONE;
				epmem_time_id high_id = EPMEM_MEMID_NONE;
				epmem_time_id *next_id;
				unsigned long next_list;				
				...
				// initialize next low
				if ( epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_GET_LOW_RANGES ], EPMEM_TIMER_QUERY_EXEC ) == SQLITE_ROW )				
				...
				epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_GET_HIGH_RANGES ], EPMEM_TIMER_QUERY_EXEC );				
				...
				do
				{
					...
					if ( !done )
					{
						...
						if ( epmem_exec_query( my_agent, my_agent->epmem_statements[ next_list ], EPMEM_TIMER_QUERY_EXEC ) == SQLITE_ROW )						
			...
			// remove indexes
			epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_RANGE_INDEX_1R ], EPMEM_TIMER_QUERY_EXEC );
			epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_RANGE_INDEX_2R ], EPMEM_TIMER_QUERY_EXEC );			
			...
			epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_TRUNCATE_WEIGHTS ], EPMEM_TIMER_QUERY_WEIGHTS );
			epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_TRUNCATE_RANGES ], EPMEM_TIMER_QUERY_EXEC );			
						...
						if ( (*wmes) != NULL )
						{
							for ( j=0; j<len; j++ )
							{
								...
								if ( (*wmes)[j]->value->common.symbol_type != IDENTIFIER_SYMBOL_TYPE )
								{
									...
									sqlite3_bind_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID ], 1, my_hash );
									sqlite3_bind_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID ], 2, parent_id );
									sqlite3_bind_text( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID ], 3, (const char *) (*wmes)[j]->attr->sc.name, -1, SQLITE_STATIC );
									switch( (*wmes)[j]->value->common.symbol_type )
									{
										case SYM_CONSTANT_SYMBOL_TYPE:
											sqlite3_bind_text( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID ], 4, (const char *) (*wmes)[j]->value->sc.name, -1, SQLITE_STATIC );
											break;
								            
										case INT_CONSTANT_SYMBOL_TYPE:
			        						sqlite3_bind_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID ], 4, (*wmes)[j]->value->ic.value );
											break;
							
										case FLOAT_CONSTANT_SYMBOL_TYPE:
			        						sqlite3_bind_double( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID ], 4, (*wmes)[j]->value->fc.value );
											break;
									}
									
									if ( sqlite3_step( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID ] ) == SQLITE_ROW )
										leaf_ids[i].push_back( epmem_create_leaf_node( sqlite3_column_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID ], 0 ), wma_get_wme_activation( my_agent, (*wmes)[j] ) ) );
								...
								else
								{
									...
									sqlite3_bind_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID_NULL ], 1, my_hash );
									sqlite3_bind_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID_NULL ], 2, parent_id );
									sqlite3_bind_text( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID_NULL ], 3, (const char *) (*wmes)[j]->attr->sc.name, -1, SQLITE_STATIC );

									if ( sqlite3_step( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID_NULL ] ) == SQLITE_ROW )
									{
										parent_syms.push( (*wmes)[j]->value );
										parent_ids.push( sqlite3_column_int64( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_H_FIND_ID_NULL ], 0 ) );
									}
...
void epmem_install_memory( agent *my_agent, Symbol *state, epmem_time_id memory_id )
{
	...
	{
		//////////////////////////////////////////////////////////////////////////
		epmem_start_timer( my_agent, EPMEM_TIMER_INSTALL_WM );
		//////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////////
		epmem_stop_timer( my_agent, EPMEM_TIMER_INSTALL_WM );
		//////////////////////////////////////////////////////////////////////////
	...
	if ( indexing == EPMEM_INDEXING_BIGTREE_INSTANCE )
	{
		...
		while ( epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_I_GET_EPISODE ], EPMEM_TIMER_INSTALL_NODES ) == SQLITE_ROW )
		{
			//////////////////////////////////////////////////////////////////////////
			epmem_start_timer( my_agent, EPMEM_TIMER_INSTALL_WM );
			//////////////////////////////////////////////////////////////////////////
	...
	else if ( indexing == EPMEM_INDEXING_BIGTREE_RANGE )
	{
		...
		while ( epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_R_GET_EPISODE ], EPMEM_TIMER_INSTALL_NODES ) == SQLITE_ROW )
		{
			//////////////////////////////////////////////////////////////////////////
			epmem_start_timer( my_agent, EPMEM_TIMER_INSTALL_WM );
			//////////////////////////////////////////////////////////////////////////
	...
	else if ( indexing == EPMEM_INDEXING_BIGTREE_RIT )
	{
		...
		while ( epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_RIT_GET_EPISODE ], EPMEM_TIMER_INSTALL_NODES ) == SQLITE_ROW )
		{
			//////////////////////////////////////////////////////////////////////////
			epmem_start_timer( my_agent, EPMEM_TIMER_INSTALL_WM );
			//////////////////////////////////////////////////////////////////////////
	...
	{		
		//////////////////////////////////////////////////////////////////////////
		epmem_start_timer( my_agent, EPMEM_TIMER_INSTALL_NODES );
		//////////////////////////////////////////////////////////////////////////
		...
		if ( !epmem_ids.empty() )
		{
			...
			//////////////////////////////////////////////////////////////////////////
			epmem_stop_timer( my_agent, EPMEM_TIMER_INSTALL_NODES );
			//////////////////////////////////////////////////////////////////////////

			while ( epmem_exec_query( my_agent, sel, EPMEM_TIMER_INSTALL_NODES ) == SQLITE_ROW )			
			{
				//////////////////////////////////////////////////////////////////////////
				epmem_start_timer( my_agent, EPMEM_TIMER_INSTALL_WM );
				//////////////////////////////////////////////////////////////////////////
...
void epmem_rit_clear_left_right( agent *my_agent )
{
	epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_RIT_TRUNCATE_LEFT ], EPMEM_TIMER_INSTALL_NODES );
	sqlite3_reset( my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_RIT_TRUNCATE_LEFT ] );

	epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_RIT_TRUNCATE_RIGHT ], EPMEM_TIMER_INSTALL_NODES );
...
void epmem_rit_add_left( agent *my_agent, epmem_time_id min, epmem_time_id max )
{
	...
	epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_RIT_ADD_LEFT ], EPMEM_TIMER_INSTALL_NODES );	
...
void epmem_rit_add_right( agent *my_agent, epmem_time_id id )
{
	...
	epmem_exec_query( my_agent, my_agent->epmem_statements[ EPMEM_STMT_BIGTREE_RIT_ADD_RIGHT ], EPMEM_TIMER_INSTALL_NODES );
