...
void action_record::print_chunk_action(action* pAction, int lActionCount)
{
    ...
    if (pAction->type == FUNCALL_ACTION)
    {
        ...
        outputManager->printa_sf(thisAgent, ")%-(");
        print_rhs_instantiation_value(pAction->id, instantiated_pref->rhs_funcs.id, instantiated_pref->identities.id, instantiated_pref->identity_sets.id->super_join->identity, false);
        outputManager->printa(thisAgent, " ^");
        print_rhs_instantiation_value(pAction->attr, instantiated_pref->rhs_funcs.attr, instantiated_pref->identities.attr, instantiated_pref->identity_sets.attr->super_join->identity, false);
        outputManager->printa(thisAgent, " ");
        print_rhs_instantiation_value(pAction->value, instantiated_pref->rhs_funcs.value, instantiated_pref->identities.value, instantiated_pref->identity_sets.value->super_join->identity, false);
        outputManager->printa_sf(thisAgent, " %c", preference_to_char(pAction->preference_type));
        if (pAction->referent)
        {
            print_rhs_instantiation_value(pAction->referent, instantiated_pref->rhs_funcs.referent, instantiated_pref->identities.referent, instantiated_pref->identity_sets.referent->super_join->identity, false);
        }
...
void action_record::print_instantiation_action(action* pAction, int lActionCount)
{
    ...
    if (pAction->type == FUNCALL_ACTION)
    {
        ...
        outputManager->printa_sf(thisAgent, "%d:%-(", lActionCount);
        print_rhs_instantiation_value(pAction->id, NULL, instantiated_pref->identities.id, instantiated_pref->identity_sets.id->super_join->identity, true);
        outputManager->printa(thisAgent, " ^");
        print_rhs_instantiation_value(pAction->attr, NULL, instantiated_pref->identities.attr, instantiated_pref->identity_sets.attr->super_join->identity, true);
        outputManager->printa(thisAgent, " ");
        print_rhs_instantiation_value(pAction->value, NULL, instantiated_pref->identities.value, instantiated_pref->identity_sets.value->super_join->identity, true);
        outputManager->printa_sf(thisAgent, " %c", preference_to_char(pAction->preference_type));
        if (pAction->referent)
        {
            print_rhs_instantiation_value(pAction->referent, NULL, instantiated_pref->identities.referent, instantiated_pref->identity_sets.referent->super_join->identity, true);
        }
        outputManager->printa_sf(thisAgent, ")%-(");
        print_rhs_instantiation_value(pAction->id, instantiated_pref->rhs_funcs.id, instantiated_pref->identities.id, instantiated_pref->identity_sets.id->super_join->identity, false);
        outputManager->printa(thisAgent, " ^");
...
//#ifndef SOAR_RELEASE_VERSION
    ...
    outputManager->printa_sf(thisAgent, "Learned partially-operational justification                        %-%u\n", stats.ungrounded_justifications_added);
