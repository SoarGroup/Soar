...
double SMem_Manager::lti_calc_base(uint64_t pLTI_ID, int64_t time_now, uint64_t n, uint64_t activations_first)
{
    ...
    if (recent_time != 0)
    {
        inhibition_odds = pow(1+pow(double(recent_time) / 10.0,-1.0));
...
double SMem_Manager::lti_activate(uint64_t pLTI_ID, bool add_access, uint64_t num_edges, double touches, bool increment_timer)
{
    ...
    if (add_access)
    {
         ...
         * tests below..
         */
        std::packaged_task<std::tuple<bool,bool>()> pt_ProhibitCheck([] {
        ...
        if (prohibited)
        {
            ...
            if (dirty)
            {
                std::packaged_task<double()> pt_historyRemove([this] {
                    double prevN = 0;
            ...
            std::packaged_task<void()> pt_prohibitReset([this] {
    ...
    std::packaged_task<void()> updateLTIAccess([this,time_now,pLTI_ID] {
    ...
    if (new_activation == SMEM_ACT_LOW || new_activation == 0.0)
    {
        ...
        new_base = pow(double(smem_max_cycle),double(-decay));
