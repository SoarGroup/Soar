...
public:
    ...
    /**
     * @brief Check if the database contains data
     *
     *  This is the equivalent of a `SELECT count(*) FROM sqlite_master WHERE type='table'` statement and then
    ...
    /**
     * @brief Prints the given table into a string.
     *
     *  This will try to look up the given table, format it into a string, and then return it.
     *
     * @return the print formatted table
     *
     * @throw Throws SQLite::Exception if the table doesn't exist.
     */
    std::string printTable(const std::string& table_name);

    /**
     * @brief Backs up the database to a file
     *
     *  This will try to backup the database to the given file.
     *
     * @throw Throws SQLite::Exception if the backup fails.
     */
