...
// Clone the provided SQL statement to this SQLite Database Connection
Statement::Statement(Database& aDatabase, const Statement& aQuery) :
    mQuery(aQuery.mQuery),
    mStmtPtr(aDatabase.mpSQLite, mQuery), // prepare the SQL query, and ref count (needs Database friendship)
    mColumnCount(0),
    mbOk(false),
    mbDone(false)
...
:   mQuery(std::move(other.mQuery)),
    mStmtPtr(std::move(other.mStmtPtr)),
    mColumnCount(std::move(other.mColumnCount)),
    mColumnNames(std::move(other.mColumnNames)),
    mbOk(std::move(other.mbOk)),
    mbDone(std::move(other.mbDone))
{}
...
Statement::Ptr::Ptr(Ptr&& other)
: mpSQLite(std::move(other.mpSQLite)),
mpStmt(std::move(other.mpStmt)),
mpRefCount(std::move(other.mpRefCount))
{}

Statement::Ptr& Statement::Ptr::operator=(Ptr&& other)
{
    mpSQLite = std::move(other.mpSQLite);
    mpStmt = std::move(other.mpStmt);
    mpRefCount = std::move(other.mpRefCount);
