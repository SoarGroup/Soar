			      ...
			      bool monitorable_only)
{
  ...
  for (ct = 1; ct < limit; ct++)
    {
      thisAgent->OutputManager->print( "%s: ", soar_callback_enum_to_name(static_cast<SOAR_CALLBACK_TYPE>(ct), false));
					...
					SOAR_CALLBACK_TYPE ct)
{
  ...
  for (c = thisAgent->soar_callbacks[ct];
       ...
       c = c->rest)
    {
	  ...
	  thisAgent->OutputManager->print( "%s ", cb->id.c_str());
    }
}
		...
		SOAR_CALLBACK_TYPE callback_type)
{
  ...
  if (head == NULL)
    {
      thisAgent->OutputManager->print( "Attempt to remove non-existant callback.\n");
      return;
    }
  ...
  if (   (callback_type == PRINT_CALLBACK)
      && (head->rest == NULL))
    {
      thisAgent->OutputManager->print( "Attempt to remove last print callback. Ignored.\n");
      return;
    }
