...
void add_preference_to_tm (agent* thisAgent, preference *pref) 
{
   ...
   if (pref->type == NUMERIC_INDIFFERENT_PREFERENCE_TYPE) {
     preference* p;
     slot* s;

     // each numeric preference should only contribute to one operator
     // acceptable preference, so should also only be linked to one
     assert(pref->next_numeric == NIL && pref->prev_numeric == NIL);

     /* search for the acceptable preference for the operator, it should be the
      * one that was copied to the btinfo.trace field of the condition to be
      * backtraced over */
     for (s = pref->id->id.slots; s; s = s->next) {
         for (p = s->preferences[ACCEPTABLE_PREFERENCE_TYPE]; p; p = p->next) {
           if (p->id == pref->id && 
               p->attr->sc.common_symbol_info.symbol_type == SYM_CONSTANT_SYMBOL_TYPE&&
               strcmp(p->attr->sc.name, "operator") == 0 &&
               p->value == pref->value)
           {
             // then this is the acceptable preference generated by the proposal?
             if (p->next_numeric) {
               insert_at_head_of_dll(pref->next_numeric, pref, next_numeric, prev_numeric);
             }
             else {
               p->next_numeric = pref;
               pref->prev_numeric = p;
