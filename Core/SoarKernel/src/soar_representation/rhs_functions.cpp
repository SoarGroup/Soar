...
Symbol* dont_learn_rhs_function_code(agent* thisAgent, cons* args, void* /*user_data*/)
{
    ...
    if (! member_of_list(state, thisAgent->explanationBasedChunker->chunk_free_problem_spaces))
    {
        ...
        // Remove the state from the (force-learn) list
        if (thisAgent->explanationBasedChunker->ebc_settings[SETTING_EBC_ONLY])
        	free_list(thisAgent, extract_list_elements(thisAgent, &(thisAgent->explanationBasedChunker->chunky_problem_spaces), cons_equality_fn, reinterpret_cast<void*>(state)));
    }
    return NIL;
