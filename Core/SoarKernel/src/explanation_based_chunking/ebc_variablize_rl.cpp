...
action* Explanation_Based_Chunker::variablize_rl_action(action* pRLAction, struct token_struct* tok, wme* w, double & initial_value)
{
    ...
    rhs->id = allocate_rhs_value_for_symbol(thisAgent, id_sym, lRS->inst_identity, lRS->cv_id, NULL, was_unbound_vars.id);
    lRS = rhs_value_to_rhs_symbol(pRLAction->attr);
    rhs->attr = allocate_rhs_value_for_symbol(thisAgent, attr_sym, lRS->inst_identity, lRS->cv_id, NULL, was_unbound_vars.attr);
    lRS = rhs_value_to_rhs_symbol(pRLAction->value);
    rhs->value = allocate_rhs_value_for_symbol(thisAgent, val_sym, lRS->inst_identity, lRS->cv_id, NULL, was_unbound_vars.value);
    lRS = rhs_value_to_rhs_symbol(pRLAction->referent);
    rhs->referent = allocate_rhs_value_for_symbol(thisAgent, ref_sym, lRS->inst_identity, lRS->cv_id, NULL, was_unbound_vars.referent);
