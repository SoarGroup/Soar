...
void Repair_Manager::mark_states_WMEs_and_store_variablizations(condition* pCondList, tc_number tc)
{
    ...
    for (lCond = pCondList; lCond; lCond = lCond->next)
    {
        if (lCond->type == POSITIVE_CONDITION)
        {
            ...
            if (lMatchedSym)
            {
                thisAgent->explanationBasedChunker->add_sti_variablization(lMatchedSym, lSym, lCond->data.tests.id_test->eq_test->inst_identity, lCond->data.tests.id_test->eq_test->chunk_inst_identity);
            }
            ...
            {
                thisAgent->explanationBasedChunker->add_sti_variablization(lMatchedSym, lSym, lCond->data.tests.value_test->eq_test->inst_identity, lCond->data.tests.id_test->eq_test->chunk_inst_identity);
            }
        }
    }
}
...
void Repair_Manager::repair_rule(condition*& p_lhs_top, matched_symbol_list* p_dangling_syms)
{
    ...
    {
        lDanglingSymInfo = *it;
        thisAgent->explanationBasedChunker->add_sti_variablization(lDanglingSymInfo->instantiated_sym, lDanglingSymInfo->variable_sym, lDanglingSymInfo->inst_identity, lDanglingSymInfo->cv_id);
    }
