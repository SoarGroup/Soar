...
uint64_t Explanation_Based_Chunker::variablize_rhs_symbol(rhs_value &pRhs_val, tc_number lti_link_tc)
{
    ...
    if (rs->identity_set)
    {
        ...
        found_variablization = get_variablization(rs->identity_set);
    }
    else
    {
    ...
    if (!found_variablization && rs->referent->is_sti())
    {
        ...
        found_variablization = store_variablization(rs->identity_set, var, rs->referent);
    }
...
action* Explanation_Based_Chunker::variablize_result_into_action(preference* result, tc_number lti_link_tc)
{
    ...
    uint64_t lIdentity = 0, lIdentitySet = 0;
