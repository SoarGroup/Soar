...
inline bool get_id_set(id_to_id_map* identity_set_mappings, uint64_t pIdentity)
{
    assert(identity_set_mappings->find(pIdentity) != identity_set_mappings->end());
    return identity_set_mappings->find(pIdentity)->second;
}
