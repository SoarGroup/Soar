...
// Making these public as this is basically just a struct.
    ...
    public:
        ...
        ~JavaCallbackData()
        {
            ...
            if (m_HandlerObject)
            {
                penv->DeleteGlobalRef(m_HandlerObject) ;
            }
            if (m_CallbackData)
            {
                penv->DeleteGlobalRef(m_CallbackData) ;
            }
            if (m_AgentObject)
            {
                penv->DeleteGlobalRef(m_AgentObject) ;
            }
            if (m_KernelObject)
            {
                penv->DeleteGlobalRef(m_KernelObject) ;
            }
        }
} ;
...
JNIEXPORT bool JNICALL Java_sml_smlJNI_Agent_1UnregisterForRunEvent(JNIEnv* jenv, jclass jcls, jlong jarg1, jlong jarg2)
{
    ...
    if (!IsValidCallbackData(pJavaData))
    {
        return false;
    }
