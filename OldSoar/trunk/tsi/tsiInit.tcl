...
proc tsi {{hideController 0} args } {
   global interp_name interp_agentSocket tsi_version tsi_library \
...
##      puts "Calibrating System Clock: $tsiConfig(calibrateClock) seconds please..."
   ...
   puts "TSI ver. $tsi_version, expertise: $tsiConfig(expertise), \
...
proc tsiInitAgent {} {
   ...
   switch $tsiConfig(interfaceType) {
      1 {
         set tsiConfig(tswFrame) .tsw
         if [catch {makeSoarInteractionWindow $X $Y} msg] {
      	   error "Failed to create Soar agent interaction window: $msg"
         }
      }
      3 {
         # Add Glenn's debugger init here
      }
      default {
