...
class SoarOperatorNode extends FileNode {
	...
	// if this SoarOperatorNode is highlevel then this is a string representing just the folder name, else it is null
	protected String folderName;

	 // if this SoarOperatorNode is highlevel then this is the Associated state in the datamap, else it is null
	protected SoarIdentifierVertex dataMapId;

	// this is the number associated with the dataMapId, if the dataMapId is null, then this is left unintialized, or 0
	...
	/*
	 * This represents the set of actions that should be preformed when an operator becomes
	 * a high-level operator
	 */
	public void firstTimeAdd(OperatorWindow operatorWindow, SoarWorkingMemoryModel swmm) throws IOException {
		// Create the Folder
	...
	public DataFlavor isDropOk(int action,DataFlavor[] dataFlavors) {
		if(action == java.awt.dnd.DnDConstants.ACTION_LINK || action == java.awt.dnd.DnDConstants.ACTION_MOVE) {
	...
	/**
	 * Given a Writer this writes out a description of the high level operator node
	 * that can be read back in later
	 * @param w the writer 
	 * @throws IOException if there is an error writing to the writer
	 */
...
*/
	...
	public OperatorNode addSuboperator(OperatorWindow operatorWindow, SoarWorkingMemoryModel swmm, String newOperatorName) throws IOException {
		...
		on = operatorWindow.createSoarOperatorNode(newOperatorName,rules.getName());
    ...
    if(!isHighLevel)
      firstTimeAdd(operatorWindow,swmm);
		...
		ion = operatorWindow.createImpasseOperatorNode(newOperatorName,rules.getName());
	...
	public void delete(OperatorWindow operatorWindow) {
		if(isHighLevel) {
			if(!linkNodes.isEmpty()) {
				int selOption = JOptionPane.showConfirmDialog(MainFrame.getMainFrame(),
								                             "This Operator has links, which will be deleted do you want to continue?",
												  		     "Link Deletion Confirmation", JOptionPane.YES_NO_OPTION);
	...
	public void notifyDeletionOfChild(OperatorWindow operatorWindow,OperatorNode child) {
		if (getChildCount() == 0) {
			...
			if(!linkNodes.isEmpty()) {
				JOptionPane.showMessageDialog(MainFrame.getMainFrame(),"This node is no longer high-level, links will be deleted.","Link deletions",JOptionPane.INFORMATION_MESSAGE);
				List linkNodesCopy = (List)((LinkedList)linkNodes).clone();
				Iterator i = linkNodesCopy.iterator();
				while(i.hasNext()) {
					LinkNode linkNodeToDelete = (LinkNode)i.next();
    ...
    DataMap dataMap;
    SoarIdentifierVertex dataMapParent = new SoarIdentifierVertex(dataMapId.getValue());
    ...
    if(dataMapIdNumber != 0) {
      ...
      if(this instanceof FileOperatorNode) {
        ...
        while( (( parent.getStateIdVertex()).getValue() != 0) && (!(parent instanceof OperatorOperatorNode)) ) {
	...
	public boolean move(OperatorWindow operatorWindow,OperatorNode newParent) {
		if(newParent instanceof SoarOperatorNode && !((SoarOperatorNode)newParent).isHighLevel()) {
			try {
				((SoarOperatorNode)newParent).firstTimeAdd(operatorWindow,operatorWindow.getDatamap());
			}
			catch(IOException ioe) {
