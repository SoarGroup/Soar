 ...
 *
 *
 *    This file is included by all of soar's kernel files.  Once a
 *   build is completed, you may determine the options it was built
 *   with using the two core api commands "soar_ecBuildInfo" and
 *   "soar_ecExcludedBuildInfo" it is expected that some compile time
 *   flags will be more informative than others.  These are listed in
 *   using the first command.  All remaining compile time flags have
 *   been explicitly hidden using comments in the source code.  It is
 *   expected that they will be of no particular use, but for
 *   completeness, they can be view using the second command
...
/**
 *  \def MATCHTIME_INTERRUPT
 * 
 *       Setting this option enables Soar to recognize productions
 *       with ':interrupt'.  Such production interrupt the agent's execution
 *       as soon as they match.  This is in constrast to the right-hand side
 *       function '(interrupt)' which interrupts execution when the production
 *       fires.  Matchtime interrupts add a minor cost to each production
 *       match.
 */
 ...
 *       This option may add a small run time cost, since two loops are made
 ...
 */
/* #define COUNT_KERNEL_TIMER_STOPS */
 ...
 *
 *       this option will tell Soar to print a warning if any of
 *    Soar's timers report zero time (given that the agent's
 *    'warnings' system parameter is set to true).  In such cases,
 *    there is a high likely hood that the timing data will
 *    misrepresent the truth because of abnormally high granularity.
 *    Using this option may slow the system down slightly (only
 *    slightly it adds approx 10 lines of code to the timing
 *    functions), however, two builds which differ only in their use
 *    of this option should similarly have equally fast (or slow)
 *    timer responses.  Thus, if consistently no warnings are
 *    generated, it should be safe to run the version withou this
 *    option and have full confidence of the results.
 *
 */
...
/*  The options may make soar run faster, but remove some
