...
#	error "Could Not Change to Directory '$path' to Load Eater : $msg"
#	
	#return
    #}
   ...
   # initEaterOutputFile $name $color $newname
...
#          changed to fix bugzilla bug 391
   ...
   if [catch {$newname eval \
              [list uplevel #0 source [file join simulator agent.tcl]]} msg] {
      tk_dialog .error Warning \
...
proc tickIO { agents } {
    global soarTimePerTick soarTimeUnit localAgents remoteAgents \
       sio_agentStatus tsiConfig agentUpdate

    if [info exists localAgents] {
	if { $tsiConfig(sioDebug) > 4 } { puts "++ Soar Tick I/O..." }
	[lindex $localAgents 0] eval \
