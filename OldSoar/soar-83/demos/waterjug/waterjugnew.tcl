...
switch $action {
	3pour5 	{
		for {set i $vol5} {$i <= 5} {incr i} {			
			if {$vol3 >=0} {
				set filename images/$action-$vol3$vol5.gif
				timer $filename
	...
	fill3 {
		set filename images/$action-$vol5.gif
		timer $filename


		}
	fill5 {
		set filename images/$action-$vol3.gif
		timer $filename
		}
	state  {

		set filename images/$vol3$vol5.gif
		timer $filename
		}
	}
}
