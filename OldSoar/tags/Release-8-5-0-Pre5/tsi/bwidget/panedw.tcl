...
proc PanedWindow::_beg_move_sash { path num x y } {
    ...
    #puts "_beg_move_sash called with $path $num $x $y"
...
proc PanedWindow::_move_sash { top min max v form } {
    ...
    #puts "_move_sash called with $top $min $max $v $form"
    if { $v < $min } {
	set v $min
    } elseif { $v > $max } {
	set v $max
...
proc PanedWindow::setPercentage { path newPercent } {

   set fprev $path.f0
   set fnext $path.f1
   set wsash [expr [Widget::getoption $path -width] + 2*[Widget::getoption $path -pad]]
   set w1    [winfo height $fprev]
   set w2    [winfo height $fnext]
   set minszg [Widget::getoption $fprev -minsize]
   set minszd [Widget::getoption $fnext -minsize]
   ...
   set newY [expr round($ymin + (($ymax - $ymin) * (double($newPercent)/100)))]
   _end_move_sash $path $path.sash 1 $ymin $ymax $newY rooty height
...
proc PanedWindow::_end_move_sash { path top num min max v rootv size } {
   ...
   #puts "_end_move_sash called with $min $max $v"
   destroy $top
   if { $v < $min } {
   	set v $min
   } elseif { $v > $max } {
   ...
   set panePercentage [expr (double($v - $min)/($max - $min))*100]
