...
void stop_timer( unsigned long long int *tv_with_start_time,
		 unsigned long long int *tv_with_accumulated_time ) {
...
void stop_timer (struct timeval *tv_with_recorded_start_time,
  ...
  if ( tv_with_accumulated_time == &(current_agent(total_kernel_time)) &&
    ...
    current_agent(kt_histogram_tv)[current_agent(d_cycle_count)].tv_sec 
      += delta_sec;
    current_agent(kt_histogram_tv)[current_agent(d_cycle_count)].tv_usec
      += delta_usec;
    
    if (current_agent(kt_histogram_tv)[current_agent(d_cycle_count)].tv_usec
	>= ONE_MILLION) {
