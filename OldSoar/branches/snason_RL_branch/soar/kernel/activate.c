 ...
 * Overhauled by Andrew Nuxoll 2003
 *  
 * 
 ...
 *
 *
 * These decay routines are stored in other files:
...
//#define DECAY_DEBUG
#ifdef DECAY_DEBUG
    #define log_activation(w,x,y,z) print_with_symbols(w, x,y,z)
    #define log_prod_name(x,y) print(x,y)
#else
...
void decay_init_quickboost(void)
{
    int history_iter;
        ...
        el.history_count++)
    {
        ...
        avg /= NUM_ITER;
        current_agent(decay_quick_boost[el.history_count]) = avg;
...
void decay_init(void)
{
    ...
    start_timer(&current_agent(decay_tv));

	init_memory_pool (&current_agent(decay_element_pool),
                      sizeof(wme_decay_element), "wme_decay");
    

	current_time = current_agent(d_cycle_count);

	// set up the array
	for(i=0; i<DECAY_ARRAY_SIZE; i++)
    {
		temp_timelist = &current_agent(decay_timelist[i]);
	...
	current_agent(current_decay_timelist_element) = &current_agent(decay_timelist[0]);
    ...
    {
        current_agent(decay_power_array)[i] = (float) pow((double) i, (double) (current_agent(sysparams)[WME_DECAY_EXPONENT_SYSPARAM] / DECAY_EXPONENT_DIVISOR));
    }
...
void decay_reference_wme(wme *w)
{
    ...
    else if (current_agent(sysparams)[WME_DECAY_I_SUPPORT_MODE_SYSPARAM] == DECAY_I_SUPPORT_MODE_NONE)
    {
        return;
    }
    ...
    while(c != NIL)
    {
        ...
        {
            decay_reference_wme(c->bt.wme); // recurse
        }
...
void decay_update_new_wme(wme *w, int num_refs)
{
    ...
    start_timer(&current_agent(decay_tv));
    start_timer(&current_agent(decay_new_wme_tv));
        ...
        if ( (w->preference != NIL) && (w->preference->o_supported != TRUE) )
        {
            switch(current_agent(sysparams)[WME_DECAY_I_SUPPORT_MODE_SYSPARAM])
            {
                ...
                case DECAY_I_SUPPORT_MODE_UNIFORM:
                    decay_reference_wme(w);
                    break;
            }
        }
        
        stop_timer(&current_agent(decay_tv), &current_agent(total_decay_time));
        stop_timer(&current_agent(decay_new_wme_tv), &current_agent(total_decay_new_wme_time));
    ...
    {
        temp_el->log = allocate_memory (sizeof(decay_log),
    ...
    temp_el->next = current_agent(current_decay_timelist_element)->first_decay_element;
    temp_el->previous = NIL;
    temp_el->time_spot = current_agent(current_decay_timelist_element);
    if(temp_el->next != NIL)
    {
        temp_el->next->previous = temp_el;
    }
    current_agent(current_decay_timelist_element)->first_decay_element = temp_el;
...
void activate_wmes_in_inst(instantiation *inst)
{
         ...
         cond=cond->next)
    {
        if (cond->type==POSITIVE_CONDITION)
        {
            ...
            decay_reference_wme(cond->bt.wme);
        }
    }
...
void decay_update_wmes_tested_in_prods()
{
     ...
     start_timer(&current_agent(decay_tv));
     start_timer(&current_agent(decay_lhs_tv));
     ...
     for (msc=current_agent(ms_o_assertions); msc!=NIL; msc=msc->next)
     {
         ...
         while (!(t == current_agent(dummy_top_token)))
         {
             if (t->w != NIL)
             {
                 log_activation("\nMS O Assert: (%y ^%y %y) ",
                                t->w->id, t->w->attr, t->w->value);
                 decay_reference_wme(t->w);
             }             
             t = t->parent;
         }//while
     }//for
     ...
     if (current_agent(sysparams)[WME_DECAY_PERSISTENT_ACTIVATION_SYSPARAM])
     {
         for (msc=current_agent(ms_retractions); msc!=NIL; msc=msc->next)
         {
                  ...
                  cond=cond->next)
             {
                 ...
                 if (cond->type==POSITIVE_CONDITION)
                 {
                     if (cond->bt.wme->decay_element != NIL)
                     {
                         log_activation("\nMS  Retract: (%y ^%y %y) ",
                                            cond->bt.wme->id,
                                            cond->bt.wme->attr,
                         ...
                         cond->bt.wme->decay_element->num_references--;
                     } 
                 }
             }//for
         }//for
     ...
     stop_timer(&current_agent(decay_tv), &current_agent(total_decay_time));
     stop_timer(&current_agent(decay_lhs_tv), &current_agent(total_decay_lhs_time));
...
void decay_update_wmes_in_retracted_inst(instantiation *inst)
{
    ...
    start_timer(&current_agent(decay_tv));
    start_timer(&current_agent(decay_rhs_tv));

    if ( (current_agent(sysparams)[WME_DECAY_SYSPARAM])
         && (current_agent(sysparams)[WME_DECAY_PERSISTENT_ACTIVATION_SYSPARAM]) )
    {
    ...
    stop_timer(&current_agent(decay_tv), &current_agent(total_decay_time));
    stop_timer(&current_agent(decay_rhs_tv), &current_agent(total_decay_rhs_time));
...
/* ============================================================================
   ...
   ========================================================================= */
void forget_wme_due_to_decay(wme *w)
{
    wme *w2;
    slot *s;
    Symbol *id;
...
/* ============================================================================
   ...
   Created:  05 August 2003
   ========================================================================= */
void decay_reposition_wme(wme_decay_element *cur_decay_el, long decay_spot)
{
    ...
    cur_decay_el->next = current_agent(decay_timelist[decay_spot]).first_decay_element;
    cur_decay_el->previous = NIL;
    cur_decay_el->time_spot = &current_agent(decay_timelist[decay_spot]);
    ...
    current_agent(decay_timelist[decay_spot]).first_decay_element = cur_decay_el;
...
/* ============================================================================
   ...
   ========================================================================= */
void decay_print_boost_info(wme *w, long prev_spot, long new_spot)
{
    int i;
    ...
    {
        print("%ld,", w->decay_element->boost_history[i]);
    }

    print(") #ref=%ld from %ld to %ld (%ld)",
          w->decay_element->num_references,
...
/* ============================================================================
   ...
   ========================================================================= */
void decay_log_boost(wme_decay_element *el, float act, int cycle)
{
    int i;
    ...
    if (cycle - el->log->start >= el->log->size)
    {
        ...
        new_act_log = allocate_memory(sizeof(float)*new_size,
                                      MISCELLANEOUS_MEM_USAGE);
        ...
        {
            free_memory(el->log->act_log, MISCELLANEOUS_MEM_USAGE);
        }
...
/* ============================================================================
   ...
   Created:  05 August 2003
   ========================================================================= */
long decay_boost_wme(wme_decay_element *cur_decay_el)
{
    long decay_spot;
    long time_iter;
...
/* ===================================================================
   ...
   =================================================================== */
void decay_print_most_activated_wmes(int n)
{
    ...
    print("END OF ACTIVATED WME LIST");
...
void decay_move_and_remove_wmes(void)
{
    ...
    start_timer(&current_agent(decay_tv));
    start_timer(&current_agent(decay_move_remove_tv));
    ...
    start_position = current_agent(current_decay_timelist_element)->position;
    ...
    for(array_iter=0; array_iter<DECAY_ARRAY_SIZE; array_iter++)
    {
        ...
        cur_decay_el = current_agent(decay_timelist[array_position]).first_decay_element;
     ...
     */
    cur_decay_el = current_agent(current_decay_timelist_element)->first_decay_element;
    while(cur_decay_el != NIL)
    {
        next = cur_decay_el->next; // save this pointer before it is deallocated
        forget_wme_due_to_decay(cur_decay_el->this_wme);
        cur_decay_el = next;
    }//while
    ...
    current_agent(current_decay_timelist_element)->time = 
        current_agent(current_decay_timelist_element)->time + DECAY_ARRAY_SIZE;
    ...
    // memory leaks
    current_agent(current_decay_timelist_element)->first_decay_element = NIL;

    // Update current position in the array.
    current_agent(current_decay_timelist_element) = 
...
void decay_write_log(wme *w)
{
    ...
    f = (FILE *)(current_agent(sysparams)[WME_DECAY_LOGGING_SYSPARAM]);
    if (f == 0) return;
    start_redirection_to_file(f);
    ...
    for(i = el->log->start;
        ...
        i++)
    {
        print("%f\t", el->log->act_log[i - el->log->start]);
    }//for
...
{
    start_timer(&current_agent(decay_tv));
    start_timer(&current_agent(decay_deallocate_tv_2));
    start_timer(&current_agent(decay_deallocate_tv));
    ...
    stop_timer(&current_agent(decay_tv), &current_agent(total_decay_time));
    stop_timer(&current_agent(decay_deallocate_tv), &current_agent(total_decay_deallocate_time));
    stop_timer(&current_agent(decay_deallocate_tv_2), &current_agent(total_decay_deallocate_time_2));
...
void decay_remove_element(wme *w)
{
    ...
    if (w->decay_element->log != NIL)
    {
        ...
        {
            free_memory(w->decay_element->log->act_log, MISCELLANEOUS_MEM_USAGE);
        }

        free_memory(w->decay_element->log, MISCELLANEOUS_MEM_USAGE);
    }
   
	free_with_pool (&current_agent(decay_element_pool), w->decay_element);
