...
void soar_default_create_agent_procedure (char * agent_name) {
  ...
  */
  ...
  current_agent(alpha)                              = 0.5;  // SAN
  current_agent(gamma)                              = 1.0;  // SAN
  current_agent(Temp)                               = 50.0;   // SAN
  ...
  current_agent(top_dir_stack) = 
    (dir_stack_struct *) malloc(sizeof(dir_stack_struct));   /* AGR 568 */
  
  current_agent(top_dir_stack)->directory = 
    (char *) malloc(MAXPATHLEN*sizeof(char));   /* AGR 568 */
...
//  _asm int 3;
  ...
  soar_invoke_global_callbacks( soar_agent, GLB_AGENT_CREATED,
				(soar_call_data) soar_agent );

  /* 
   * If this is not the first agent created, reset the current agent
   * pointer
   */
  if ( curr_agent != NULL ) {  soar_agent = curr_agent; }
