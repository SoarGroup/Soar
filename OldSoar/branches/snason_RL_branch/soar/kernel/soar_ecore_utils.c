...
void capture_input_wme( enum captured_action_type action, 
  ...
  case ADD_WME:
    fprintf(current_agent(capture_fileID),
			"%ld : %d : add-wme : %ld", current_agent(d_cycle_count), ADD_WME,
			sw->timetag );

    fprintf(current_agent(capture_fileID), " : %s %s %s : ", 
			sw->id, sw->attr, sw->value );

	fprintf(current_agent(capture_fileID), "%s ", 
			  symbol_to_string( w->id, TRUE, NULL ));

	fprintf(current_agent(capture_fileID), "%s ", 
			symbol_to_string( w->attr, TRUE, NULL ));
  ...
  case REMOVE_WME:
    fprintf(current_agent(capture_fileID),
	    "%ld : %d : remove-wme :   : %ld\n",
	    current_agent(d_cycle_count), REMOVE_WME,
	    sw->timetag);	  
    break;
...
void replay_input_wme(agent * agent, soar_callback_data dummy, 
  ...
  if ( mode != (soar_call_data) NORMAL_INPUT_CYCLE) return;
  ...
  for( c_action = current_agent(replay_actions)[current_agent(d_cycle_count)];
    ...
    case REMOVE_WME: 
      ...
      soar_cRemoveWmeUsingTimetag(
         current_agent(replay_timetags)[((soarapi_wme *)c_action->args)->timetag]);
      break;
