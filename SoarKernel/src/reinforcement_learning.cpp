  ...
  while( goal )
    {
      ...
      data->current_op_rl_rules->clear();
  ...
  if ( !using_gaps || op_rl )
      ...
      if ( !data->prev_op_rl_rules->empty() )
	{
	  ...
	  // For each prod with a trace, perform update
	  {
		...
		// calculate updates
		delta_ecr = ( alpha * 1.0 * ( rule_reward - old_ecr) );
...
void rl_perform_update_new( agent *my_agent, double op_value, bool op_rl, Symbol *goal, preference *candidates, bool update_efr )
{
  bool using_gaps = ( my_agent->rl_params->temporal_extension->get_value() == soar_module::on );
  ...
  if ( !using_gaps || op_rl )
      ...
      if ( !data->prev_op_rl_rules->empty() )
	{
	  if ( my_agent->sysparams[ TRACE_RL_SYSPARAM ] )
	    {
	      ...
	      for ( rl_rule_list::iterator p=data->prev_op_rl_rules->begin(); p!=data->prev_op_rl_rules->end(); p++ )
		{
		  std::cout << "new_rl: Examining instantiation of " << (*p).first->name->sc.name << ":";
	      ...
	      std::cout << std::endl << std::endl;
	      // print out the current fired rules 
	      for ( rl_rule_list::iterator p=data->current_op_rl_rules->begin(); p!=data->current_op_rl_rules->end(); p++ )
		{
		  std::cout << "Currently fired " << (*p).first->name->sc.name << ":";
	  ...
	  if ( data->gap_age && using_gaps && my_agent->sysparams[ TRACE_RL_SYSPARAM ] )
	    {
	      char buf[256];
	      SNPRINTF( buf, 254, "new_rl:gap ended (%c%llu)", goal->id.name_letter, static_cast<long long unsigned>(goal->id.name_number) );
	  ...
	  // trace_increment is calculated for a set of productions with same labels
	  rl_reward_map trace_increments;
	  rl_reward_map trace_counts;
	  rl_reward_map sum_old_ecrs;
	  rl_reward_map sum_old_efrs;


	  for (rl_reward_map::iterator r_itr = data->rewards->begin(); r_itr != data->rewards->end(); r_itr++)
		{
		  sum_old_ecrs[ r_itr->first ] = 0.0;
		  sum_old_efrs[ r_itr->first ] = 0.0;
		  trace_increments[ r_itr->first ] = 0.0;
		  trace_counts[ r_itr->first ] = 0.0;
		} 

	  if ( !data->prev_op_rl_rules->empty() )
	    {
				
	      //number of productions per label
	      for( rl_rule_list::iterator p=data->prev_op_rl_rules->begin(); p!=data->prev_op_rl_rules->end(); p++ )
		{
		  if ( (*p).first != NIL )
		    {
		      for ( rl_label_set::iterator l_itr = (*p).second.begin(); l_itr != (*p).second.end(); l_itr++ )
			{
			  trace_counts [ (*l_itr) ] += 1.0;
			}
		    }
		}
			       
	      //compute trace increment
	      for ( rl_reward_map::iterator itr = trace_counts.begin(); itr != trace_counts.end(); itr++ )
		{
		  trace_increments[ itr->first ] = ( 1.0 / itr->second );
		}
		  ...
		  if ( (*p).first != NIL )
		    {
		      ...
		      double et_trace_increment = 0;
						
		      iter = data->eligibility_traces->find( (*p).first );
						
		      // sum over all the labels in the production and add all the increments. 
		      // Will most likely work only if there is one reward label per production
		      for (rl_label_set::iterator itr = (*p).second.begin(); itr != (*p).second.end(); itr++)
			{
			  sum_old_ecrs[ (*itr) ] += (*p).first->rl_ecr;
			  sum_old_efrs[ (*itr) ] += (*p).first->rl_efr;
			  et_trace_increment += trace_increments[ (*itr) ];
			}
						
		      if ( iter != data->eligibility_traces->end() ) 
			{
			  iter->second += et_trace_increment ;
			}
		      else 
			{
			  (*data->eligibility_traces)[ (*p).first ] = et_trace_increment;
			}
		    }
		}
	    }

	  // for the operator currently selected, compute the numeric preference attributed to each reward label
	  rl_reward_map current_numerics;
	  current_numerics.clear();
	  for (rl_reward_map::iterator r_itr = data->rewards->begin(); r_itr != data->rewards->end(); r_itr++)
		{
		  current_numerics[ r_itr->first ] = 0.0;
		} 
	  if (candidates != NULL)
	    {
	      rl_collect_current_op_rules (my_agent, goal, candidates);			
	      for ( rl_rule_list::iterator c_itr = data->current_op_rl_rules->begin(); c_itr != data->current_op_rl_rules->end(); c_itr++)
		{
		  for (rl_label_set::iterator l_itr = c_itr->second.begin(); l_itr != c_itr->second.end(); l_itr++)
		    {
		      current_numerics[ (*l_itr) ] += ( c_itr->first->rl_ecr + c_itr->first->rl_efr );
		    }
		}
	    }
	  ...
	  // For each prod with a trace, perform update
	  {
	    ...
	    for ( iter = data->eligibility_traces->begin(); iter != data->eligibility_traces->end(); iter++ )
	      {	
		production *prod = iter->first;
		double production_old_ecr = 0;
		double production_old_efr = 0;
		double production_current_numeric = 0;
					
		old_ecr = prod->rl_ecr;
		old_efr = prod->rl_efr;

		//find the label set associated with the current production and sum up the corresponding rewards
		rule_reward = 0.0;
		for (rl_rule_list::iterator p = data->prev_op_rl_rules->begin(); p != data->prev_op_rl_rules->end(); p++)
		  {
		    if ((*p).first == prod)
			...
			for (rl_label_set::iterator it = (*p).second.begin(); it != (*p).second.end(); it++)
			  {
			    rule_reward += data->rewards->find( *it )->second;
			    production_old_ecr += sum_old_ecrs.find( *it )->second;
			    production_old_efr += sum_old_efrs.find( *it )->second;
			    production_current_numeric += current_numerics.find( *it )->second;
			  }
		      }
		  }
		
		if ( my_agent->sysparams[ TRACE_RL_SYSPARAM ] )
		  std::cout << "rule name " << prod->name->sc.name << " trace:" << iter->second << "  rule_reward:" << rule_reward << " production_old_ecr:" << production_old_ecr << " production_current_numeric " << production_current_numeric << " production_old_efr " << production_old_efr << std::endl; 


		delta_ecr = ( alpha * iter->second * ( rule_reward - production_old_ecr) );
					
		if ( update_efr )
		  {
		    delta_efr = ( alpha * iter->second * ( ( discount * production_current_numeric ) - production_old_efr ) );
		  }
		else
		  {
		    delta_efr = 0.0;
...
void rl_collect_current_op_rules( agent *my_agent, Symbol *goal, preference *cand)
{
  ...
  if ( my_agent->sysparams[ TRACE_RL_SYSPARAM ] )
    std::cout<< "The operator currently selected is " << op->id.name_letter << op->id.name_number << std::endl;
  ...
  for ( preference *pref = goal->id.operator_slot->preferences[ NUMERIC_INDIFFERENT_PREFERENCE_TYPE ]; pref; pref = pref->next )
	...
	{			
	  ...
	  data->current_op_rl_rules->push_back( std::make_pair< production*, rl_label_set >( pref->inst->prod, labels ) );
	}
    }
}
