...
inline void epmem_process_buffered_wmes( agent* my_agent, Symbol* state, soar_module::wme_set& cue_wmes, soar_module::symbol_triple_list& my_list, epmem_wme_stack* epmem_wmes )
{
	if ( my_list.empty() )
	{
		return;
...
void epmem_init_db( agent *my_agent, bool readonly )
{
	...
	else
	{
...

		{
			...
			// FIXME JUSTIN rebuild pool-pool maps

			(*my_agent->epmem_id_repository)[ EPMEM_NODEID_ROOT ] = new epmem_hashed_id_pool;
			(*my_agent->epmem_id_siblings)[ EPMEM_NODEID_ROOT ] = EPMEM_NODEID_ROOT;
			(*my_agent->epmem_wm_tree)[ EPMEM_NODEID_ROOT ] = new epmem_hash_id_map;
			{
