...
for (int i=0; i<names.size(); ++i) {
  ...
  builders["node_" + name] = {
    node(name + " && unix") {
      ...
      sh "export VERISON=\$(<soarversion'); 7za a \${VERSION}-" + nodeName + ".7zip out/"
    ...
    node(name + " && windows") {
      ...
      sh 'set /p VERSION=<soarversion; "C:/Program Files/7-Zip/7z.exe" a %VERSION%-%BUILD_ID%-' + nodeName + '-VS2015.7zip out/'
