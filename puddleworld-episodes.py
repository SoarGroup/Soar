...
def main():
  if len(sys.argv) == 1:
    ...
    while True:
      ...
      if not line or line == '':
        ...
        split = line.split(' ')
        if last_ep == int(split[1]):
          cumulative_reward += float(split[3])
        else:
          if last_ep != 0:
            smith.append(cumulative_reward)
          last_ep = int(split[1])
          cumulative_reward = float(split[3])
    f.close()
      ...
      while not done:
        ...
        for handle in files[group].handles:
          ...
          if not line or line == '':
            ...
            split = line.split(' ')
            if last_eps[handle] == int(split[1]):
              cumulative_rewards[handle] += float(split[3])
            else:
              if last_ep != 0:
                vals.append(cumulative_rewards[handle])
              last_eps[handle] = int(split[1])
              cumulative_rewards[handle] = float(split[3])
        if not done:
          vals = sorted(vals)
  ...
  pylab.ylabel('Reward (Mvng Avg, n=20)', fontsize=8)
